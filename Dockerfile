...
# .dockerignore keeps .git out of the context, so the build is told what git would have said.
ARG OPENTS_STAMP_COMMIT=
ARG OPENTS_STAMP_BRANCH=
ARG OPENTS_STAMP_DATE=
ARG OPENTS_STAMP_DIRTY=0
...
RUN --mount=type=cache,id=opents-build,target=/src/build \
 ...
 && emcmake cmake -S . -B build -G Ninja \
        ...
        -DOPENTS_STAMP_COMMIT="$OPENTS_STAMP_COMMIT" \
        -DOPENTS_STAMP_BRANCH="$OPENTS_STAMP_BRANCH" \
        -DOPENTS_STAMP_DATE="$OPENTS_STAMP_DATE" \
        -DOPENTS_STAMP_DIRTY="$OPENTS_STAMP_DIRTY" \
