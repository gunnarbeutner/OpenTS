...
The display options screen of the WebAssembly build is where the setting is chosen there. It
offers Automatic and the four fixed steps in place of a list of resolutions, since the frame
in that build follows the browser window and there is no display mode to pick. Accepting a
size lays the screen out again at the size it already is, which is what puts the new
division in place; a resolution change is what does that anywhere else.
