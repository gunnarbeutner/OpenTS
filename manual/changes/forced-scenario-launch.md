...
`-SCENARIO=GDI1A.MAP -CAMPAIGN=GDI1`. Menu selection without these options is unchanged.
`-WINAFTER=<frame>` wins the current mission on the game frame named, so the score screen,
the campaign map and the next briefing can be reached without playing a mission through.
