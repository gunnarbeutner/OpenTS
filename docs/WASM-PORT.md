...
its image, and a declared run is believed at once, opens its window at once,
reaches at least 4 MiB and at least four times the last read whatever the link
is measured to be worth, and never past the end of its file. The end it is held to is whichever declaration covers the
read it is at rather than the one it was begun with, since the file layer
settles a bias in several steps and a run carries on out of one embedded file
into the next. An extent covering the whole image declares nothing, because it
is hinted every time the image is opened and the engine opens an archive far
more often than it reads one through; registering an archive reads a few
hundred bytes of index, and a declaration would put that read at the front of
a whole window. An undeclared run has to be noticed, which costs two forward
