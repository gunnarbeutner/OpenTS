...
platform layer it builds, how it reads game data over HTTP, and the design of
each subsystem behind that. It also keeps the judgements that
...
not return. A scan of `code/*.cpp`, made while the owner-draw dialogs still
existed, for loop bodies that service `Sleep`, `Call_Back`,
`Windows_Message_Handler` or `OwnerDraw::Dialog_Message_Handler` found about
ninety loops in thirty-odd files. Roughly nine were incidental (the frame pacer
`Sync_Delay`, the `while (!GameInFocus)` spins, the dialog reveal wipe), about
eighteen were bounded iterations that only service the callback while doing
finite work, and the remaining sixty-odd were genuine waits on external state:
a peer's packets, a click, an audio stream ending, a whole movie playing inside
one call.

The waits nest across subsystems. `Main_Loop` runs a screen that runs
`Main_Loop` (`UI_Service_Game`, capped at one level by a static guard); a
lockstep stall runs a whole frame inside `Wait_For_Players`; a desync opens a
modal message box from inside `Queue_AI`; the lobby's waits in
`code/windlg.cpp` and `code/netdlg2.cpp` run handlers that open further
screens. The two raw message pumps left, `Windows_Message_Handler` and the
movie player's `VQA_Message_Handler`, pump only on Windows; on the page both
take in the page's events through `Browser_Service` instead, and every other
wait reaches one of them, which is what makes the thread recoverable from so
few places.

### 1.1 What was built

`main` in `code/startup.cpp` calls `Browser_Init`, which attaches the engine to
the canvas and the page's event callbacks, and then carries on with the same
startup every target runs. `main` returns
...
mostly) spin on it alone and never reach the message handler. A screen over a
single-player game runs neither pacer: `UI_Service_Game` pumps the queue and
calls `Call_Back`, whose yield is paced, so the pass yields once itself and the
loop waits a frame instead of spinning through it.

On this target `Windows_Message_Handler` has two jobs it does not have on
Windows: it hands what the page reported to the game (`Browser_Service`) and
paints when a repaint was asked for (`Host_Take_Paint`), and it hands the
thread back before it leaves. It also services the audio backend, because the
sound driver's own pass runs only from `Call_Back` and the loading and waiting
stretches never reach it, and it runs `Video_Service_Display`, because it is
the one pump the movie player and the screen loops all come through.
...
| File I/O | `FileClass` and its `RawFileClass` specialization sit on `PlatformFileClass` in `code/platform`, whose POSIX implementation is [the file layer](PLATFORM.md#2-the-file-layer) a page reads through. Nothing above `FileClass` changed. |
...
| Mouse cursor | `code/wincursor.cpp` turns a shape frame into a host cursor, which the page shows as its own pointer; see [5.5](#55-the-cursor). |
| Input | A push interface shared with Windows: keys go to `Keyboard->Post_Key_Event` and mouse buttons to `Game_Window_Mouse_Button` in `code/gamewindow.cpp`, from the page's callbacks here and from the window procedure there. The keyboard asks `code/hostwindow.h` for the modifiers, held keys and typed character. A page reports modifiers with each event and has no key state to ask afterwards, so `code/browser.cpp` holds them; the browser reports the character with the event, so `Host_Key_To_Character` reads back what was recorded as the key went down. Buttons are never swapped in `Host_Key_Is_Down`, because the desktop has applied that setting before the event names a button. |
| Audio | miniaudio's Web Audio backend under `AudioDeviceClass`; see [section 6](#6-audio-and-movies). |
| Time | `mstimer.cpp` and `milsectmr.cpp` read `std::chrono::steady_clock`; `Platform_Sleep` yields to the page ([waits](PLATFORM.md#3-waits-and-the-process)). |
| Strings | Compiled into the module from `code/languagestrings.cpp`, as on every target; see [strings](PLATFORM.md#5-strings-and-version-text). |
...
The page builds the POSIX half of the platform layer, and
[The platform layer](PLATFORM.md) is its document: the layout and ABI
settings, the file layer, waits and the process, the game window, strings, and
exceptions. What this page adds is the page's half of each seam: the image
overlay the file layer answers out of ([section 4](#4-game-data-over-http)),
the canvas the game window is ([section 5](#5-display-input-and-the-shell)),
and the yields the waits make ([section 1](#1-the-thread-and-the-waits)).
...
or not at all, which is what makes it fit to store. A read is cut where what
the store holds ends, so the held part of it is served locally and the request
pays only for the rest; a read straddling that edge would otherwise fetch
again everything a window had already brought down. Fetched blocks are kept in
...
was noticed. The file layer declares one for the extent a file occupies inside
its image, and a declared run is believed from its first read rather than its
second and never reaches past the end of its file. The end it is held to is
whichever declaration covers the read it is at rather than the one it was begun
with, since the file layer settles a bias in several steps and a run carries on
out of one embedded file into the next. An extent covering the whole image
declares nothing, because it is hinted every time the image is opened and the
engine opens an archive far more often than it reads one through; a run inside
one has to be noticed like any other.

A declaration says where a file ends, not that the file is going to be read
through, so neither kind of run reaches further in front of the reading than
the reading has covered: a declared run four times as far, and at least four
times its last read whatever the link is measured to be worth, an undeclared
one as far and only once it has been noticed, which costs two forward reads.
Sizing a declared run's window by the link instead swept a megabyte or more off
the front of every archive nested inside another, since registering one is an
open that declares the whole of it followed by a read of its directory and
nothing else. Reaching the main menu asked for 13.9 MB of archive, 8.6 MB of it
in four such sweeps, and asks for 6.3 MB now.
...
against the size they came up at), and during a present. A shell
...
On a page the game window is the canvas (`code/hostwindow_page.cpp`), so a
size change has no window to move. The presenter follows the canvas at once by
scaling the frame it already holds, so the picture is never absent while the
window is dragged. The list of display modes the engine can enumerate
(`Host_Display_Mode`) is the canvas's own size, the current mode, and the
familiar 4:3 and widescreen sizes that fit the display, in CSS pixels, rebuilt
at the start of each enumeration.
...
than notches, so travel is accumulated and `Game_Window_On_Mouse_Wheel` is
called once per notch's worth, or a trackpad's stream of small deltas would fly
the build list past what the player reached for. A scroll over the tactical view pans the map
...
composition, and delivered as a press and release of the US layout key that
...
property, so `Host_Create_Cursor` turns the game's shape into a PNG data URL
the page draws, and the cursor never touches a game surface. A page scales the
image by the device pixel ratio itself and refuses one over 128 pixels,
showing nothing rather than a clipped one, so the image is built at the largest
whole scale that fits. Setting no cursor gives the canvas the page's default
pointer, and `Host_Hide_Cursor` sets `none`. Windows restores a window's class
cursor on the next mouse move; a page has no class cursor, so the game's own
...
has no playback loop of its own; its sound is fed by the mixer's pass in
...
checks that it begins with the `OTSV` signature, and returns the bytes for
`Pick_Filename` to name and `RawFileClass` to write, which also gets the IndexedDB flush that any write
through the file layer already triggers. Import needs no selection and the load
option is offered even when no save exists, or the one screen that can bring a
...
`SocketClass` in `code/netsocket.h` is the seam under that framing: the
routing header, the send queue, the retry logic and the game are written once
above it. A page can neither broadcast nor open a datagram socket, so
`code/wsrelay.cpp` supplies a `SocketClass` that carries datagrams over a
WebSocket to [the relay](RELAY.md) in place of a network, one server
forwarding by the recipient id the header already carries; `docs/RELAY.md`
owns the protocol. `IPXManagerClass::Configure_LAN` hands that socket to an
ordinary `UDPInterfaceClass`, and seats the client on the relay before doing
so, because the tunnel header names this client by the id the relay hands out
and carries it from the first packet on.
...
look like the version mismatch it is. `RelaySocketClass::Open` waits for the
greeting, yielding to the page, bounded at ten seconds, because the engine
sets its network up in one call. Message arrivals are queued rather than
announced, because the page's event loop can run while the engine is suspended
inside a fetch, and the transport's service pass drains that queue as it would
drain a socket. A relayed broadcast arrives naming the broadcast id rather
than the receiving player, so the interface registers that id as its own. Cross-play
...
The port needs no threads, and the reason matters. On Windows the engine
starts a thread to keep audio fed while the main thread blocks
(`AudioFeederClass`), and the crash reporter has threads of its own. A page
runs the feeder's passes from the engine's waits instead
([6.1](#61-the-backend)), and the crash reporter is Windows-only, so the
logger's `std::mutex`, the one lock the engine's portable code takes, is only
ever taken by one thread. So the build needs no pthreads, no
`SharedArrayBuffer` and no cross-origin isolation, can be served from any
static host, and keeps testing deterministic.
...
- A page cannot move the pointer (`Host_Move_Pointer`), so the edge-scroll warp
  does nothing.
