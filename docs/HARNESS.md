...
| `ui [path]` | What is on screen, as the engine describes it: the phase, the menu items, the gadgets, the objects. Printed as one line, written as JSON when a path is given. |
...
| `text <string>` | Typed one key at a time. A character with no key on a US keyboard is sent with an empty `code`, as another layout reports it. |
| `skip-movies [seconds]` | Sends escape to each film in turn until the engine is in a phase that stays: a menu, a load, an alert, or the game. |
...
queue and the keyboard buffer both empty (`OpenTS_Input_Pending`) -- and one
more frame has been drawn, up to `--settle` seconds (three by default). The step's result says whether that happened
...
and the page shows it as `OpenTS_State.phases`, such as `loading/movie`. The top
...
| `@7` | A menu item or a gadget by its number. |
| `text:"Resume Mission"` | A gadget by the text it shows, compared without case. |
...
line saying what is: the phase, the menu page and its items, the gadget count
and the object types. The click lands on the centre of the target's rectangle.
...
under the pointer. `windows` is always empty: every screen that was a window
is drawn by the UI shell, which the description does not cover, so a step
reaches a control on one of those screens by its position. The gadgets are the
...
(`code/platform/file_posix.cpp`'s `Host_Path`), so this is what an installed settings
