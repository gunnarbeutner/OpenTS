"""Serves one build directory and the asset tree beside it.

The engine reads its game data out of the archives an asset tree names, with
HTTP range requests, and refuses a server that answers with the whole file
instead, so the one thing this server must get right is ``206`` and a
``Content-Range`` that can be read back.  ``http.server`` answers no range at
all, which is why this is here rather than a line of shell.

The asset tree is named into the URL space rather than copied or linked into
the build directory: it stays wherever OpenTS-Assets wrote it, and nothing the
harness serves ever lands inside the repository.
"""
...
class _Handler(http.server.BaseHTTPRequestHandler):
    ...
    def _resolve(self):
        ...
        # A named file answers before the build directory does, so a build that
        # happens to hold a copy of its own is still served the one that was named.
        if name in self.extras:
            return self.extras[name]

        # The asset tree is laid out the way a deployment serves it: the pointer
        # at the root and everything else under assets/.
        if self.assets is not None and (name == "assets.json" or name.startswith("assets/")):
            candidate = os.path.join(self.assets, name)
    ...
    def _serve(self, body):
        ...
        if header:
            ...
            if start >= total or length <= 0:
                ...
                self.end_headers()
                if self.record is not None:
                    self.record(self.path, 416, start, 0)
                return
        ...
        if self.record is not None:
            self.record(self.path, status, start, length)
...
class BuildServer:
    ...
    def __init__(self, root, assets=None, extras=(), host="127.0.0.1"):
        ...
        if assets is not None:
            self.assets = os.path.abspath(os.path.expanduser(assets))
            if not os.path.isfile(os.path.join(self.assets, "assets.json")):
                raise FileNotFoundError("no assets.json in the asset tree: %s" % self.assets)

        self.extras = {}
        for extra in extras:
            path = os.path.abspath(os.path.expanduser(extra))
            if not os.path.isfile(path):
                raise FileNotFoundError("no such file to serve: %s" % path)
    ...
    def _record(self, path, status, start, length):
        with self._lock:
            self.requests.append((time.monotonic() - self._started, path, status, start, length))
    ...
    def summary(self):
        ...
        served = {}
        for at, path, status, start, length in requests:
            key = path.split("?", 1)[0]
    ...
    def raw(self):
        """Every request, in the order it arrived, for a run that wants the shape of its
        own traffic rather than just the totals -- whether a prefetch is asking for
        consecutive small ranges instead of the one request a whole file would take."""

        with self._lock:
            requests = list(self.requests)

        return [
            {"at": round(at, 3), "path": path.split("?", 1)[0], "status": status,
