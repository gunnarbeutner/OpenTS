...
class Browser:
    ...
    def __init__(self, path, width, height, scale=1.0, headless=True, autoplay=True,
                 extra=()):
        self.path = path
        ...
        # A run that is not about sound wants the films to play without being asked, which
        # is what every other run relies on. Leaving the policy alone instead is what shows
        # a run what a visitor gets: a page that makes no sound until it is interacted with.
        if autoplay:
            arguments.append("--autoplay-policy=no-user-gesture-required")
