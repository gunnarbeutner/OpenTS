...
def build_stamp():
    """What the banner says a build came from, as the image cannot read the repository itself.

    The build context excludes .git, which is what keeps it small, so the values git would have
    answered are passed in. A tree that is not a repository yields an empty stamp, and the build
    reports what it does not know, exactly as a source archive does.
    """

    def ask(*args):
        try:
            out = subprocess.run(["git", "-C", str(ROOT)] + list(args), capture_output=True,
                                 text=True)
        except OSError:
            return ""
    ...
    return {
        "OPENTS_STAMP_COMMIT": commit,
        "OPENTS_STAMP_BRANCH": "" if branch == "HEAD" else branch,
        "OPENTS_STAMP_DATE": ask("log", "-1", "--format=%cd", "--date=format:%Y-%m-%d %H:%M:%S"),
        "OPENTS_STAMP_DIRTY": "1" if ask("status", "--porcelain") else "0",
    }
...
def build_images(options):
    ...
    for name, value in build_stamp().items():
        engine += ["--build-arg", "%s=%s" % (name, value)]
