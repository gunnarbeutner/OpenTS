	...
	// browser's database. code/platform/file_posix.cpp resolves a save into this directory and asks for it
	...
	// Whether a shell is hosting this page rather than a browser tab, which it says by
	// passing "hosted=1".
	function OpenTS_Is_Shell() {
		return (new URLSearchParams(window.location.search).get("hosted") || "") === "1";
	}
	...
	function OpenTS_Offer_Downloads() {
		...
		// is shown for either and the sections inside it appear on their own terms. The
		// offline half is set up by the caller, once its entry points answer.

		// Neither reason for withholding an installer reaches keeping the game, so the two
		// gates sit on the build list rather than on the panel.
		if (!OpenTS_Is_Shell() && !OpenTS_Is_Handheld()) {
		...
		// Inside the app there is nothing left to keep but the data.
		if (OpenTS_Is_Shell()) {
			labelElement.textContent = "Play offline";
			openElement.title = "Play offline";
		}
	...
	function OpenTS_Say_Durability(noteElement) {
		...
		navigator.storage.persisted().then(function (granted) {
			if (granted) {
				noteElement.textContent += " It is kept until you clear it.";
				return;
			}

			// Telling a reader to install the page as an app is no advice inside the app.
			noteElement.textContent += OpenTS_Is_Shell() ?
				" The system may clear it if the device runs short of space." :
	...
	function OpenTS_Keep_Streamed(noteElement) {
		// A shell registers no worker, so there is no cache for these to land in.
	...
	function OpenTS_Show_Downloads() {
		...
		// The offline entry points are not always exported by the first frame, so this
		// waits for them rather than offering a section that would set nothing up.
		if (!offlineOffered && OpenTS_Offline_Available()) {
			offlineOffered = true;
