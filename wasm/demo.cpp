...
/*
 * A standalone WebAssembly target that drives the renderer seam and nothing
 * else. The browser owns the loop: one call advances the demo by a bounded
 * number of frames and returns, the pattern docs/WASM-PORT.md describes.
 */
...
// Presenting the native frame into a canvas of another size puts the scaling
// path under test.
static const int GAME_WIDTH = 640;
static const int GAME_HEIGHT = 480;

// The shell page gives the canvas this id.
static char const * const CANVAS_SELECTOR = "#canvas";

// The simulation rate; presentation is not tied to it because the browser
// decides when a frame is composited.
static const double FRAME_INTERVAL = 1000.0 / 60.0;

// A throttled tab comes back with arbitrary arrears, and working through all
// of it would block.
...
// What a blocking engine would hold on the C++ stack.
...
// The scale modes need a hard edge to filter; a plasma alone hides the
...
static void Render_Pattern(unsigned short * frame, unsigned int tick)
{
	...
	// Point sampling keeps these one pixel lines whole; linear filtering
	// blurs them.
	...
	// Corner markers make a wrong destination rectangle visible.
...
// Fits the frame inside the canvas the way video.cpp fits it in the client
// area.
...
// A page resizes for reasons the engine never sees, so the drawable size is
// read back each pass. Layout is in CSS pixels and the drawing buffer in
// device pixels; conflating them makes a frame look soft.
...
// A hidden tab is not composited and its callback may not run at all.
...
// Always returns; the result tells a paced skip from a suspended page.
static FrameResult Demo_Frame(void)
{
	...
	// The simulation catches up to a bound and then resynchronizes, since
	// working through the whole arrears would block.
	int advanced = 0;
	...
	// Presentation is unconditional: a browser that composites an undrawn
	// frame shows a cleared buffer.
	Backend_Present(_Frame, GAME_WIDTH * (int)sizeof(unsigned short), _DestX, _DestY, _DestWidth, _DestHeight, _ScaleMode, true);
	Backend_End_Frame();
	_PresentCount++;
...
static void Demo_Main_Loop(void)
{
	...
	// Updating the status line is a call into JavaScript, so it runs on a
	// slow cadence.
...
// Lets the page exercise all three filters without a rebuild.
...
// Tells an automated check that the loop is running, not merely started.
...
int main(void)
{
	...
	// A page has only WebGL 2, which bgfx reaches through OpenGL ES; vsync is
	// the browser's to decide.
	// bgfx names a canvas by CSS selector, so the selector goes where a platform hands over
	// a window handle.
	NativeWindow canvas;
	canvas.Type = NATIVE_WINDOW_DEFAULT;
	canvas.Display = nullptr;
	canvas.Handle = (void *)CANVAS_SELECTOR;

	if (!Backend_Init(canvas, _CanvasWidth, _CanvasHeight, BACKEND_RENDERER_OPENGLES, false)) {
	...
	// Zero asks for the browser's animation cadence, and main must return to
	// the browser.
