...
struct BrowserEvent
{
	...
	// The character a key press types, zero for none.
...
// The modifiers and character of the event being drained, presented while
// it is dispatched.
static unsigned short _EventModifiers = 0;
static char32_t _EventCharacter = 0;
...
static char32_t _Character[256];
static char32_t _ShiftedCharacter[256];

// Carries a character no key of the US layout types, as VK_PACKET does on Windows.
static unsigned short const VK_CHARACTER = VK_NONE_E7;
...
static void Canvas_Point_To_Game(double cssx, double cssy, int & x, int & y)
{
	...
	Point2D point((int)(cssx * ratio + 0.5), (int)(cssy * ratio + 0.5));
...
static EM_BOOL Key_Callback(int type, EmscriptenKeyboardEvent const * event, void *)
{
	...
	// A dead key starts a composition that claiming it would cancel; the key that finishes
	// it reports the composed character.
	if (strcmp(event->key, "Dead") == 0) {
		return(EM_FALSE);
	}
	...
	char32_t character = 0;
	char const * value = event->key;
	char32_t const first = (value[0] != '\0') ? UTF8::Decode(value) : 0;

	if (first != 0 && *value == '\0' && UTF8::Is_Printable(first)) {
		character = first;
	} else {
		switch (key & 0xFF) {
	...
	if (character != 0) {
		if ((_Modifiers & WWKEY_SHIFT_BIT) != 0) {
			_ShiftedCharacter[key & 0xFF] = character;
		} else {
			_Character[key & 0xFF] = character;
		}
	}
	...
	queued.Character = release ? 0 : character;
	Queue_Event(queued);
...
// The map takes the travel whole; the build list takes it a notch at a time.
static void Service_Wheel(void)
{
	...
	while (_WheelPendingY >= WHEEL_NOTCH || _WheelPendingY <= -WHEEL_NOTCH) {
		...
		// A page counts scrolling away from the player as positive and the game counts it as
		// negative.
		Game_Window_On_Mouse_Wheel(away ? -1 : 1);
	}
}
	...
	EM_ASM({
		...
		if (!el) {
			...
			el.opentsSync = function () {
				...
				/* The engine counts characters, not UTF-16 halves. */
				if (common > 0 && (value.charCodeAt(common - 1) & 0xFC00) === 0xD800) {
					common--;
				}

				var removed = Array.from(prev.slice(common));
				for (var back = 0; back < removed.length; back++) {
					el.opentsQueue.push(8);
				}
				var added = Array.from(value.slice(common));
				for (var index = 0; index < added.length; index++) {
					el.opentsQueue.push(added[index].codePointAt(0));
				}
				...
				if (value.length > 64) {
					var kept = Array.from(value).slice(-32).join("");
					el.value = kept;
					el.opentsPrev = kept;
				}
			};
...
static void Return_Text_Character(int character)
{
	EM_ASM({
		var el = document.getElementById("opents-text");
		if (el && el.opentsQueue) {
			el.opentsQueue.unshift($0);
		}
	}, character);
}
...
// key of its own borrows the US layout key that produces it, or VK_CHARACTER.
...
static void Service_Text_Input(void)
{
	...
	// A key's table entry holds one character until the engine reads it, so a character that
	// would replace a different one written in this pass waits for the next.
	char32_t written[256] = {};
	...
	for (int character = Fetch_Text_Character(); character != 0; character = Fetch_Text_Character()) {
		...
		if (key == VK_NONE) {
			if (!UTF8::Is_Printable((char32_t)character)) {
				continue;
			}
			key = VK_CHARACTER;
		}

		if (written[key & 0xFF] != 0 && written[key & 0xFF] != (char32_t)character) {
			Return_Text_Character(character);
			break;
		}
		written[key & 0xFF] = (char32_t)character;

		// Recorded under both shift states because the key is only a carrier.
		_Character[key & 0xFF] = (char32_t)character;
		_ShiftedCharacter[key & 0xFF] = (char32_t)character;
...
static EM_BOOL Touch_Callback(int type, EmscriptenTouchEvent const * event, void *)
{
	...
	if (type == EMSCRIPTEN_EVENT_TOUCHSTART) {
		...
		if (points[0] != nullptr) {
			...
			// pointer answers the finger; a screen's control draws itself
			// pressed until the release, so it is pressed at once, and
			// Touch_Service_Hold leaves such a gesture alone.
			int x;
			int y;
			Canvas_Point_To_Game(_GestureStartX, _GestureStartY, x, y);
			_MouseX = x;
			...
			if (UI_Browser_Takes_Touch_Press(x, y)) {
				Touch_Press();
			}
		}
		return(EM_TRUE);
	...
	if (type == EMSCRIPTEN_EVENT_TOUCHMOVE) {
		...
		if (_Gesture == GESTURE_UNDECIDED) {
			...
			if (travel > TOUCH_SLOP) {
				...
				// A screen's control took the press as the finger landed, and a drag
				// keeps it wherever the finger goes.
				if (_GesturePressed) {
					Touch_Begin_Drag();
				} else if (column >= 0) {
					_FlickColumn = column;
...
void Browser_Service(void)
{
	...
	while (_EventHead != _EventTail) {
		...
		// alone so it cannot reach the event hook or the game window and open
		// the options dialog behind the movie.
		...
		_EventCharacter = event.Character;
		...
		if (event.IsMouse) {
			Game_Window_Mouse_Button(event.Key, Point2D(event.X, event.Y), event.IsRelease);
...
char32_t Browser_Key_To_Character(unsigned short key)
{
	...
	if ((key & WWKEY_SHIFT_BIT) != 0 && _ShiftedCharacter[vk_key] != 0) {
		return(_ShiftedCharacter[vk_key]);
	}
	return(_Character[vk_key]);
}
...
{
	public:
		BrowserMouseClass(void) : WWMouseClass() {}
...
int Browser_Message_Box(char const * caption, char const * text, int buttons)
{
	...
	static_assert(HOST_ANSWER_OK == 1 && HOST_ANSWER_CANCEL == 2 && HOST_ANSWER_YES == 6 && HOST_ANSWER_NO == 7, "message box results");
...
bool Browser_Init(void)
{
	...
	memset(_ShiftedCharacter, 0, sizeof(_ShiftedCharacter));
	...
	// No dblclick callback: the page delivers both clicks of a double click in full as
	// mousedown and mouseup and then fires dblclick besides, so taking that as well would
	// report a click the player never made. Win32 is the other way round, where
	// WM_LBUTTONDBLCLK stands in for the second press and hostwindow_win32.cpp rebuilds
	// the click from it.
