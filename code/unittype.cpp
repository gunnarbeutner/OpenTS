...
void UnitTypeClass::Serialize(SaveStreamClass & stream)
{
	...
	SERIALIZE(stream, HalfDamageSmokeLocation);
	SERIALIZE(stream, IsPassive);
	SERIALIZE(stream, IsCrateGoodie);
	SERIALIZE(stream, IsToHarvest);
	SERIALIZE(stream, IsToVeinHarvest);
	SERIALIZE(stream, IsFireAnim);
	SERIALIZE(stream, IsLockTurret);
	SERIALIZE(stream, IsNoFireWhileMoving);
	SERIALIZE(stream, IsDeployToFire);
	SERIALIZE(stream, IsTilter);
	SERIALIZE(stream, IsUseTurretShadow);
	SERIALIZE(stream, IsTooBigToFitUnderBridge);
	SERIALIZE(stream, IsTotable);
	SERIALIZE(stream, IsSmallVisceroid);
	SERIALIZE(stream, IsLargeVisceroid);
	SERIALIZE(stream, IsCarriesCrate);
	// AltImageData -- artwork, fetched from the mix files again as this loads.
	SERIALIZE(stream, IsNonVehicle);
	SERIALIZE(stream, IsJellyfish);
	SERIALIZE(stream, IsLimpetDrone);
	SERIALIZE(stream, IsMobileEMP);
	SERIALIZE(stream, IsCoreDefender);
	SERIALIZE(stream, StandingFrames);
	SERIALIZE(stream, DeathFrames);
	SERIALIZE(stream, DeathFrameRate);
	SERIALIZE(stream, MaxCharge);
	SERIALIZE(stream, StartCharge);
	SERIALIZE(stream, FiringSyncFrame);
	SERIALIZE(stream, StartStandFrame);
	SERIALIZE(stream, StartWalkFrame);
	SERIALIZE(stream, StartFiringFrame);
	SERIALIZE(stream, StartDeathFrame);
	SERIALIZE(stream, MaxDeathCounter);
	SERIALIZE(stream, Facings);
	SERIALIZE(stream, TurretFacings);
	SERIALIZE(stream, StartTurretFrame);
	SERIALIZE(stream, WalkFrames);
	SERIALIZE(stream, FiringFrames);
	SERIALIZE(stream, AltImageFile);
}
