...
{
	SERIALIZE(stream, AmmoCrateDamage);
	SERIALIZE(stream, LargeVisceroid);
	SERIALIZE(stream, SmallVisceroid);
	SERIALIZE(stream, UnloadingHarvester);
	SERIALIZE(stream, AttackingAircraftSightRange);
	SERIALIZE(stream, TunnelSpeed);
	SERIALIZE(stream, TiberiumHeal);
	SERIALIZE(stream, HSBuilding);
	SERIALIZE(stream, IsFreeMCV);
	SERIALIZE(stream, IsBerzerkAllowed);
	SERIALIZE(stream, PoseDir);
	SERIALIZE(stream, DropPodPuff);
	SERIALIZE(stream, WaypointAnimationSpeed);
	SERIALIZE(stream, BarrelExplode);
	SERIALIZE(stream, BarrelDebris);
	SERIALIZE(stream, BarrelParticle);
	SERIALIZE(stream, RadarEventColorSpeed);
	SERIALIZE(stream, RadarEventMinRadius);
	SERIALIZE(stream, RadarEventSpeed);
	SERIALIZE(stream, RadarEventRotationSpeed);
	SERIALIZE(stream, FlashFrameTime);
	SERIALIZE(stream, RadarCombatFlashTime);
	SERIALIZE(stream, MaxWaypointPathLength);
	SERIALIZE(stream, Wake);
	SERIALIZE(stream, FlamingInfantry);
	SERIALIZE(stream, AITriggerSuccessWeightDelta);
	SERIALIZE(stream, AITriggerFailureWeightDelta);
	SERIALIZE(stream, AITriggerTrackRecordCoefficient);
	SERIALIZE(stream, VeinholeMonsterStrength);
	SERIALIZE(stream, MaxVeinholeGrowth);
	SERIALIZE(stream, VeinholeGrowthRate);
	SERIALIZE(stream, VeinholeShrinkRate);
	SERIALIZE(stream, VeinAttack);
	SERIALIZE(stream, VeinDamage);
	SERIALIZE(stream, MaximumQueuedObjects);
	SERIALIZE(stream, AircraftFogReveal);
	SERIALIZE(stream, WoodCrateImg);
	SERIALIZE(stream, CrateImg);
	SERIALIZE(stream, DropPod);
	SERIALIZE(stream, DeadBodies);
	SERIALIZE(stream, MetallicDebris);
	SERIALIZE(stream, BridgeExplosions);
	SERIALIZE(stream, DigSound);
	SERIALIZE(stream, Dig);
	SERIALIZE(stream, IonBlast);
	SERIALIZE(stream, IonBeam);
	SERIALIZE(stream, InfantryExplode);
	SERIALIZE(stream, AtmosphereEntry);
	SERIALIZE(stream, PrerequisitePower);
	SERIALIZE(stream, PrerequisiteFactory);
	SERIALIZE(stream, PrerequisiteBarracks);
	SERIALIZE(stream, PrerequisiteRadar);
	SERIALIZE(stream, PrerequisiteTech);
	SERIALIZE(stream, GateUpSound);
	SERIALIZE(stream, GateDownSound);
	SERIALIZE(stream, JumpjetTurnRate);
	SERIALIZE(stream, JumpjetSpeed);
	SERIALIZE(stream, JumpjetClimb);
	SERIALIZE(stream, JumpjetCruiseHeight);
	SERIALIZE(stream, JumpjetAcceleration);
	SERIALIZE(stream, JumpjetWobblesPerSecond);
	SERIALIZE(stream, JumpjetWobbleDeviation);
	SERIALIZE(stream, RadarEventSuppressionDistances);
	SERIALIZE(stream, RadarEventVisibilityDurations);
	SERIALIZE(stream, RadarEventDurations);
	SERIALIZE(stream, IonCannonDamage);
	SERIALIZE(stream, RailgunDamageRadius);
	SERIALIZE(stream, ZoomInFactor);
	SERIALIZE(stream, ConditionRedSparkingProbability);
	SERIALIZE(stream, ConditionYellowSparkingProbability);
	SERIALIZE(stream, TiberiumExplosionDamage);
	SERIALIZE(stream, TiberiumStrength);
	SERIALIZE(stream, MinLowPowerProductionSpeed);
	SERIALIZE(stream, MultipleFactory);
	SERIALIZE(stream, MultipleFactoryCap);
	SERIALIZE(stream, CraterLevel);
	SERIALIZE(stream, TreeFlammability);
	SERIALIZE(stream, MissileSpeedVar);
	SERIALIZE(stream, MissileROTVar);
	SERIALIZE(stream, DropPodWeapon);
	SERIALIZE(stream, DropPodHeight);
	SERIALIZE(stream, DropPodSpeed);
	SERIALIZE(stream, DropPodAngle);
	SERIALIZE(stream, ScrollMultiplier);
	SERIALIZE(stream, CrewEscape);
	SERIALIZE(stream, ShakeScreen);
	SERIALIZE(stream, HoverHeight);
	SERIALIZE(stream, HoverBob);
	SERIALIZE(stream, HoverBoost);
	SERIALIZE(stream, HoverAcceleration);
	SERIALIZE(stream, HoverBrake);
	SERIALIZE(stream, HoverDampen);
	SERIALIZE(stream, PlacementDelay);
	SERIALIZE(stream, ExplosiveVoxelDebris);
	SERIALIZE(stream, TireVoxelDebris);
	SERIALIZE(stream, ScrapVoxelDebris);
	SERIALIZE(stream, BridgeVoxelMax);
	SERIALIZE(stream, CloakingStages);
	SERIALIZE(stream, RevealTriggerRadius);
	SERIALIZE(stream, IceCrackingWeight);
	SERIALIZE(stream, IceBreakingWeight);
	SERIALIZE(stream, IceCrackSounds);
	SERIALIZE(stream, CliffBackImpassability);
	SERIALIZE(stream, VeteranRatio);
	SERIALIZE(stream, VeteranCombat);
	SERIALIZE(stream, VeteranSpeed);
	SERIALIZE(stream, VeteranSight);
	SERIALIZE(stream, VeteranArmor);
	SERIALIZE(stream, VeteranROF);
	SERIALIZE(stream, VeteranCap);
	SERIALIZE(stream, CloakSound);
	SERIALIZE(stream, SellSound);
	SERIALIZE(stream, GameClosed);
	SERIALIZE(stream, IncomingMessage);
	SERIALIZE(stream, SystemError);
	SERIALIZE(stream, OptionsChanged);
	SERIALIZE(stream, GameForming);
	SERIALIZE(stream, PlayerLeft);
	SERIALIZE(stream, PlayerJoined);
	SERIALIZE(stream, Construction);
	SERIALIZE(stream, CreditTicks);
	SERIALIZE(stream, CrumbleSound);
	SERIALIZE(stream, BuildingSlam);
	SERIALIZE(stream, RadarOn);
	SERIALIZE(stream, RadarOff);
	SERIALIZE(stream, ScoldSound);
	SERIALIZE(stream, TeslaCharge);
	SERIALIZE(stream, TeslaZap);
	SERIALIZE(stream, GenericClick);
	SERIALIZE(stream, GenericBeep);
	SERIALIZE(stream, BlowupSound);
	SERIALIZE(stream, HealCrateSound);
	SERIALIZE(stream, ChuteSound);
	SERIALIZE(stream, StopSound);
	SERIALIZE(stream, GuardSound);
	SERIALIZE(stream, ScatterSound);
	SERIALIZE(stream, DeploySound);
	SERIALIZE(stream, LightningSound);
	SERIALIZE(stream, WorstLowPowerBuildRateCoefficient);
	SERIALIZE(stream, BestLowPowerBuildRateCoefficient);
	SERIALIZE(stream, WallBuildSpeedCoefficient);
	SERIALIZE(stream, ChargeToDrainRatio);
	SERIALIZE(stream, DamageToFirestormDamageCoefficient);
	SERIALIZE(stream, TrackedUphill);
	SERIALIZE(stream, TrackedDownhill);
	SERIALIZE(stream, WheeledUphill);
	SERIALIZE(stream, WheeledDownhill);
	SERIALIZE(stream, SpotlightMovementRadius);
	SERIALIZE(stream, SpotlightLocationRadius);
	SERIALIZE(stream, SpotlightSpeed);
	SERIALIZE(stream, SpotlightAcceleration);
	SERIALIZE(stream, SpotlightAngle);
	SERIALIZE(stream, SpotlightRadius);
	SERIALIZE(stream, WindDirection);
	SERIALIZE(stream, CameraRange);
	SERIALIZE(stream, FlightLevel);
	SERIALIZE(stream, BuildingDrop);
	SERIALIZE(stream, Scorches);
	SERIALIZE(stream, Scorches1);
	SERIALIZE(stream, Scorches2);
	SERIALIZE(stream, Scorches3);
	SERIALIZE(stream, Scorches4);
	SERIALIZE(stream, Craters);
	SERIALIZE(stream, RepairBay);
	SERIALIZE(stream, GDIGateOne);
	SERIALIZE(stream, GDIGateTwo);
	SERIALIZE(stream, NodGateOne);
	SERIALIZE(stream, NodGateTwo);
	SERIALIZE(stream, WallTower);
	SERIALIZE(stream, GDIPowerPlant);
	SERIALIZE(stream, GDIPowerTurbine);
	SERIALIZE(stream, NodRegularPower);
	SERIALIZE(stream, NodAdvancedPower);
	SERIALIZE(stream, GDIFirestormGenerator);
	SERIALIZE(stream, GDIHunterSeeker);
	SERIALIZE(stream, NodHunterSeeker);
	SERIALIZE(stream, BuildConst);
	SERIALIZE(stream, BuildPower);
	SERIALIZE(stream, BuildRefinery);
	SERIALIZE(stream, BuildBarracks);
	SERIALIZE(stream, BuildTech);
	SERIALIZE(stream, BuildWeapons);
	SERIALIZE(stream, BuildDefense);
	SERIALIZE(stream, BuildPDefense);
	SERIALIZE(stream, BuildAA);
	SERIALIZE(stream, BuildHelipad);
	SERIALIZE(stream, BuildRadar);
	SERIALIZE(stream, ConcreteWalls);
	SERIALIZE(stream, NSGates);
	SERIALIZE(stream, EWGates);
	SERIALIZE(stream, GDIWallDefense);
	SERIALIZE(stream, GDIWallDefenseCoefficient);
	SERIALIZE(stream, NodBaseDefenseCoefficient);
	SERIALIZE(stream, GDIBaseDefenseCoefficient);
	SERIALIZE(stream, ComputerBaseDefenseResponse);
	SERIALIZE(stream, AIDetectDisguise);
	SERIALIZE(stream, MaximumBaseDefenseValue);
	SERIALIZE(stream, BaseUnit);
	SERIALIZE(stream, HarvesterUnit);
	SERIALIZE(stream, PadAircraft);
	SERIALIZE(stream, OnFire);
	SERIALIZE(stream, TreeFire);
	SERIALIZE(stream, Smoke1);
	SERIALIZE(stream, Smoke2);
	SERIALIZE(stream, FirestormActiveAnim);
	SERIALIZE(stream, FirestormIdleAnim);
	SERIALIZE(stream, FirestormAirAnim);
	SERIALIZE(stream, FirestormGroundAnim);
	SERIALIZE(stream, MoveFlash);
	SERIALIZE(stream, BombParachute);
	SERIALIZE(stream, Parachute);
	SERIALIZE(stream, SplashList);
	SERIALIZE(stream, SmallFire);
	SERIALIZE(stream, LargeFire);
	SERIALIZE(stream, Paratrooper);
	SERIALIZE(stream, Disguise);
	SERIALIZE(stream, Technician);
	SERIALIZE(stream, Engineer);
	SERIALIZE(stream, Pilot);
	SERIALIZE(stream, Crew);
	SERIALIZE(stream, FlameDamage);
	SERIALIZE(stream, FlameDamage2);
	SERIALIZE(stream, NukeWarhead);
	SERIALIZE(stream, NukeProjectile);
	SERIALIZE(stream, NukeDown);
	SERIALIZE(stream, EMPulseWarhead);
	SERIALIZE(stream, EMPulseProjectile);
	SERIALIZE(stream, C4Warhead);
	SERIALIZE(stream, IonCannonWarhead);
	SERIALIZE(stream, FirestormWarhead);
	SERIALIZE(stream, VeinholeWarhead);
	SERIALIZE(stream, IonStormWarhead);
	SERIALIZE(stream, VeinholeTypeClass);
	SERIALIZE(stream, DefaultLargeGreySmokeSystem);
	SERIALIZE(stream, DefaultSmallGreySmokeSystem);
	SERIALIZE(stream, DefaultSparkSystem);
	SERIALIZE(stream, DefaultLargeRedSmokeSystem);
	SERIALIZE(stream, DefaultSmallRedSmokeSystem);
	SERIALIZE(stream, DefaultDebrisSmokeSystem);
	SERIALIZE(stream, DefaultFireStreamSystem);
	SERIALIZE(stream, DefaultFirestormExplosionSystem);
	SERIALIZE(stream, DefaultTestParticleSystem);
	SERIALIZE(stream, DefaultRepairParticleSystem);
	SERIALIZE(stream, MyEffectivenessCoefficientDefault);
	SERIALIZE(stream, TargetEffectivenessCoefficientDefault);
	SERIALIZE(stream, TargetSpecialThreatCoefficientDefault);
	SERIALIZE(stream, TargetStrengthCoefficientDefault);
	SERIALIZE(stream, TargetDistanceCoefficientDefault);
	SERIALIZE(stream, DumbMyEffectivenessCoefficient);
	SERIALIZE(stream, DumbTargetEffectivenessCoefficient);
	SERIALIZE(stream, DumbTargetSpecialThreatCoefficient);
	SERIALIZE(stream, DumbTargetStrengthCoefficient);
	SERIALIZE(stream, DumbTargetDistanceCoefficient);
	SERIALIZE(stream, EnemyHouseThreatBonus);
	SERIALIZE(stream, HunterSeekerDetonateProximity);
	SERIALIZE(stream, HunterSeekerDescendProximity);
	SERIALIZE(stream, HunterSeekerDescentSpeed);
	SERIALIZE(stream, HunterSeekerAscentSpeed);
	SERIALIZE(stream, HunterSeekerEmergeSpeed);
	SERIALIZE(stream, TurboBoost);
	SERIALIZE(stream, AttackInterval);
	SERIALIZE(stream, AttackDelay);
	SERIALIZE(stream, PowerEmergencyFraction);
	SERIALIZE(stream, AirstripRatio);
	SERIALIZE(stream, AirstripLimit);
	SERIALIZE(stream, HelipadRatio);
	SERIALIZE(stream, HelipadLimit);
	SERIALIZE(stream, TeslaRatio);
	SERIALIZE(stream, TeslaLimit);
	SERIALIZE(stream, AARatio);
	SERIALIZE(stream, AALimit);
	SERIALIZE(stream, DefenseRatio);
	SERIALIZE(stream, DefenseLimit);
	SERIALIZE(stream, WarRatio);
	SERIALIZE(stream, WarLimit);
	SERIALIZE(stream, BarracksRatio);
	SERIALIZE(stream, BarracksLimit);
	SERIALIZE(stream, RefineryLimit);
	SERIALIZE(stream, RefineryRatio);
	SERIALIZE(stream, BaseSizeAdd);
	SERIALIZE(stream, PowerSurplus);
	SERIALIZE(stream, InfantryReserve);
	SERIALIZE(stream, InfantryBaseMult);
	SERIALIZE(stream, SoloCrateMoney);
	SERIALIZE(stream, CrateMoneyBonus);
	SERIALIZE(stream, IsArmorCrateStacking);
	SERIALIZE(stream, IsFirepowerCrateStacking);
	SERIALIZE(stream, TreeStrength);
	SERIALIZE(stream, UnitCrateType);
	SERIALIZE(stream, PatrolTime);
	SERIALIZE(stream, TeamDelays);
	SERIALIZE(stream, AIHateDelays);
	SERIALIZE(stream, DissolveUnfilledTeamDelay);
	SERIALIZE(stream, AIIonCannonConYardValue);
	SERIALIZE(stream, AIIonCannonWarFactoryValue);
	SERIALIZE(stream, AIIonCannonPowerValue);
	SERIALIZE(stream, AIIonCannonEngineerValue);
	SERIALIZE(stream, AIIonCannonThiefValue);
	SERIALIZE(stream, AIIonCannonHarvesterValue);
	SERIALIZE(stream, AIIonCannonMCVValue);
	SERIALIZE(stream, AIIonCannonAPCValue);
	SERIALIZE(stream, AIIonCannonBaseDefenseValue);
	SERIALIZE(stream, AIIonCannonPlugValue);
	SERIALIZE(stream, AIIonCannonHelipadValue);
	SERIALIZE(stream, AIIonCannonTempleValue);
	SERIALIZE(stream, AIAlternateProductionCreditCutoff);
	SERIALIZE(stream, MultiplayerAICreditMultipliers);
	SERIALIZE(stream, MinimumAIDefensiveTeams);
	SERIALIZE(stream, MaximumAIDefensiveTeams);
	SERIALIZE(stream, TotalAITeamCap);
	SERIALIZE(stream, AIUseTurbineUpgradeChance);
	SERIALIZE(stream, FillEarliestTeamProbability);
	SERIALIZE(stream, CloakDelay);
	SERIALIZE(stream, GameSpeedBias);
	SERIALIZE(stream, NervousBias);
	SERIALIZE(stream, ExplosionSpread);
	SERIALIZE(stream, SupressRadius);
	SERIALIZE(stream, MaxIQ);
	SERIALIZE(stream, IQSuperWeapons);
	SERIALIZE(stream, IQProduction);
	SERIALIZE(stream, IQGuardArea);
	SERIALIZE(stream, IQRepairSell);
	SERIALIZE(stream, IQCrush);
	SERIALIZE(stream, IQScatter);
	SERIALIZE(stream, IQContentScan);
	SERIALIZE(stream, IQAircraft);
	SERIALIZE(stream, IQHarvester);
	SERIALIZE(stream, IQSellBack);
	SERIALIZE(stream, AIBaseSpacing);
	SERIALIZE(stream, SilverCrate);
	SERIALIZE(stream, WoodCrate);
	SERIALIZE(stream, CrateMinimum);
	SERIALIZE(stream, CrateMaximum);
	SERIALIZE(stream, LZScanRadius);
	SERIALIZE(stream, FlareAnim);
	SERIALIZE(stream, MPMoney);
	SERIALIZE(stream, MPMaxMoney);
	SERIALIZE(stream, MPUnitCount);
	SERIALIZE(stream, MPBuildLevel);
	SERIALIZE(stream, DropZoneRadius);
	SERIALIZE(stream, MessageDelay);
	SERIALIZE(stream, SavourDelay);
	SERIALIZE(stream, MaxPlayers);
	SERIALIZE(stream, BaseDefenseDelay);
	SERIALIZE(stream, SuspendPriority);
	SERIALIZE(stream, SuspendDelay);
	SERIALIZE(stream, SurvivorFraction);
	SERIALIZE(stream, SurvivorDivisor);
	SERIALIZE(stream, ReloadRate);
	SERIALIZE(stream, AutocreateTime);
	SERIALIZE(stream, BuildupTime);
	SERIALIZE(stream, HarvesterLoadRate);
	SERIALIZE(stream, HarvesterDumpRate);
	SERIALIZE(stream, AtomDamage);
	SERIALIZE(stream, Diff);
	SERIALIZE(stream, QuakeDamagePercent);
	SERIALIZE(stream, QuakeChance);
	SERIALIZE(stream, GrowthRate);
	SERIALIZE(stream, ShroudRate);
	SERIALIZE(stream, FogRate);
	SERIALIZE(stream, IceGrowthRate);
	SERIALIZE(stream, VeinGrowthRate);
	SERIALIZE(stream, IceSolidifyDelay);
	SERIALIZE(stream, AmbientLightChangeRate);
	SERIALIZE(stream, AmbientLightChangeStep);
	SERIALIZE(stream, CrateTime);
	SERIALIZE(stream, TimerWarning);
	SERIALIZE(stream, TiberiumTransmogrify);
	SERIALIZE(stream, NukeTime);
	SERIALIZE(stream, EMPulseTime);
	SERIALIZE(stream, IonCannonTime);
	SERIALIZE(stream, FirestormTime);
	SERIALIZE(stream, SpeakDelay);
	SERIALIZE(stream, DamageDelay);
	SERIALIZE(stream, Gravity);
	SERIALIZE(stream, LeptonsPerSightIncrease);
	SERIALIZE(stream, Incoming);
	SERIALIZE(stream, MinDamage);
	SERIALIZE(stream, MaxDamage);
	SERIALIZE(stream, RepairStep);
	SERIALIZE(stream, RepairPercent);
	SERIALIZE(stream, IRepairStep);
	SERIALIZE(stream, RepairRate);
	SERIALIZE(stream, URepairRate);
	SERIALIZE(stream, IRepairRate);
	SERIALIZE(stream, SelfHealStep);
	SERIALIZE(stream, SelfHealRate);
	SERIALIZE(stream, SelfHealCap);
	SERIALIZE(stream, ConditionGreen);
	SERIALIZE(stream, ConditionYellow);
	SERIALIZE(stream, ConditionRed);
	SERIALIZE(stream, RandomAnimateTime);
	SERIALIZE(stream, CloseEnoughDistance);
	SERIALIZE(stream, StrayDistance);
	SERIALIZE(stream, CrushDistance);
	SERIALIZE(stream, CrateRadius);
	SERIALIZE(stream, HomingScatter);
	SERIALIZE(stream, BallisticScatter);
	SERIALIZE(stream, RefundPercent);
	SERIALIZE(stream, BridgeStrength);
	SERIALIZE(stream, BuildSpeedBias);
	SERIALIZE(stream, C4Delay);
	SERIALIZE(stream, RepairThreshhold);
	SERIALIZE(stream, PathDelay);
	SERIALIZE(stream, BlockagePathDelay);
	SERIALIZE(stream, MovieTime);
	SERIALIZE(stream, TiberiumShortScan);
	SERIALIZE(stream, TiberiumLongScan);
	SERIALIZE(stream, LightningFrequency);
	SERIALIZE(stream, LightningRandomness);
	SERIALIZE(stream, LightningDamage);
	SERIALIZE(stream, LightningDuration);
	SERIALIZE(stream, LightningDeferment);
	SERIALIZE(stream, CollapseChance);
	SERIALIZE(stream, WeedCapacity);
	SERIALIZE(stream, ExtraUnitLight);
	SERIALIZE(stream, ExtraInfantryLight);
	SERIALIZE(stream, ExtraAircraftLight);
	SERIALIZE(stream, IsComputerParanoid);
	SERIALIZE(stream, IsCurleyShuffle);
	SERIALIZE(stream, IsMultiMCV);
	SERIALIZE(stream, IsRecheckPrerequisites);
	SERIALIZE(stream, IsBlendedFog);
	SERIALIZE(stream, IsCompEasyBonus);
	SERIALIZE(stream, IsFineDifficulty);
	SERIALIZE(stream, IsExplosiveHarvester);
	SERIALIZE(stream, IsHealthBar);
	SERIALIZE(stream, IsAllyReveal);
	SERIALIZE(stream, IsSeparate);
	SERIALIZE(stream, IsTreeTarget);
	SERIALIZE(stream, IsNamed);
	SERIALIZE(stream, IsAutoCrush);
	SERIALIZE(stream, IsSmartDefense);
	SERIALIZE(stream, IsScatter);
	SERIALIZE(stream, IsRevealByHeight);
	SERIALIZE(stream, IsShroudedSubteranneanMovesAllowed);
	SERIALIZE(stream, IsShroudGrow);
	SERIALIZE(stream, IsMPShadowGrow);
	SERIALIZE(stream, IsMPBasesOn);
	SERIALIZE(stream, IsMPTiberiumGrow);
	SERIALIZE(stream, IsMPCrates);
	SERIALIZE(stream, IsMPAIPlayers);
	SERIALIZE(stream, IsMPCaptureTheFlag);
	SERIALIZE(stream, IsMPBridgeDestruction);
	SERIALIZE(stream, IsMPBuildOffAllyAnyStructure);
	SERIALIZE(stream, NodAIBuildsWalls);
	SERIALIZE(stream, AIBuildsWalls);
	SERIALIZE(stream, UseMinDefenseRule);
	SERIALIZE(stream, EMPulseSparkles);
	SERIALIZE(stream, WebbedInfantry);
	SERIALIZE(stream, JumpjetCloakDetectionRadius);
	SERIALIZE(stream, DropPodInfantryMinimum);
	SERIALIZE(stream, DropPodInfantryMaximum);
	SERIALIZE(stream, TalkBubbleTime);
	SERIALIZE(stream, PrerequisiteGDIFactory);
	SERIALIZE(stream, PrerequisiteNodFactory);
	SERIALIZE(stream, EngineerCaptureLevel);
	SERIALIZE(stream, EngineerDamage);
}
