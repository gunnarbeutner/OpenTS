...
class Preprocessor:
    ...
    def read_source(self, path):
        with open(path, "rb") as handle:
            data = handle.read()
        # The script names its own code page; one that names none is Windows-1252.
        return data.decode("utf-8" if b"code_page(65001)" in data else "cp1252")
    ...
    def include(self, rest, directory, number):
        ...
        # Windows SDK headers; rcimage defines their symbols itself.
        if name.lower() in ("winres.h", "afxres.h", "windows.h"):
            return

        found = self.find_include(name, directory)
        if found is None:
            raise Error("line %d: cannot find %s" % (number, name))
