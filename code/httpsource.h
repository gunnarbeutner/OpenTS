...
class BlockReadAheadClass
{
	public:
		...
		// Takes on the end of the file the reading has been found in, which a
		// run already under way may not have been begun with.
		void Bound(std::uint64_t stop);
		...
		enum {
			...
			// How far in front of the reading a declared run reaches, as a
			// multiple of what the reading has covered. A reader taking large
			// bites is therefore fetched further ahead than it consumes in one,
			// whatever the link measures, and a file read once is not swept.
			BOUND_READS = 4
		};
...
class HttpBlockSourceClass : public BlockSourceClass
{
	...
	static constexpr std::uint64_t WASTE_FLOOR = 1024ull * 1024ull;
		...
		// Blocks from one onwards that the store answers alike.
		std::uint64_t Held_Run(std::uint64_t index, std::uint64_t count) const;
