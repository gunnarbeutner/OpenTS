...
void BuildingTypeClass::Serialize(SaveStreamClass & stream)
{
	...
	SERIALIZE(stream, HalfDamageSmokeLocation1);
	SERIALIZE(stream, HalfDamageSmokeLocation2);
	SERIALIZE(stream, GateCloseDelay);
	SERIALIZE(stream, LightVisibility);
	SERIALIZE(stream, LightIntensity);
	SERIALIZE(stream, LightRedTint);
	SERIALIZE(stream, LightGreenTint);
	SERIALIZE(stream, LightBlueTint);
	SERIALIZE(stream, PrimaryFirePixelOffset);
	SERIALIZE(stream, SecondaryFirePixelOffset);
	SERIALIZE(stream, ToOverlay);
	// ToTile -- named by the rules, and resolved once the tile types are in place.
	SERIALIZE(stream, BuildupFilename);
	SERIALIZE(stream, PowersUpBuilding);
	SERIALIZE(stream, FreeUnit);
	SERIALIZE(stream, FoundationFace);
	SERIALIZE(stream, Adjacent);
	SERIALIZE(stream, ToBuild);
	SERIALIZE(stream, ExitCoordinate);
	// ExitList -- a shared exit table, re-attached by size as this loads.
	SERIALIZE(stream, StartFace);
	SERIALIZE(stream, Power);
	SERIALIZE(stream, Drain);
	SERIALIZE(stream, Size);
	SERIALIZE(stream, ZHeight);
	SERIALIZE(stream, MidPoint);
	SERIALIZE(stream, DoorStages);
	SERIALIZE(stream, Anims);
	SERIALIZE(stream, AnimData);
	SERIALIZE(stream, Upgrades);
	// DeployingAnim -- artwork, fetched from the mix files again as this loads.
	// UnderDoorAnim
	// DoorAnim
	// SpecialZOverlay
	SERIALIZE(stream, SpecialZOverlayZAdjust);
	// BibShape -- artwork, fetched from the mix files again as this loads.
	SERIALIZE(stream, NormalZAdjust);
	SERIALIZE(stream, AntiAirValue);
	SERIALIZE(stream, AntiArmorValue);
	SERIALIZE(stream, AntiInfantryValue);
	SERIALIZE(stream, ZShapePointMove);
	SERIALIZE(stream, DrawRect);
	SERIALIZE(stream, ExtraLight);
	SERIALIZE(stream, IsCanTogglePower);
	SERIALIZE(stream, HasSpotlight);
	SERIALIZE(stream, IsTemple);
	SERIALIZE(stream, IsPlug);
	SERIALIZE(stream, IsHoverPad);
	SERIALIZE(stream, IsBase);
	SERIALIZE(stream, IsBibbed);
	SERIALIZE(stream, IsWall);
	SERIALIZE(stream, IsCaptureable);
	SERIALIZE(stream, IsPowered);
	SERIALIZE(stream, IsUnsellable);
	SERIALIZE(stream, IsRadar);
	SERIALIZE(stream, IsHasChargeAnim);
	SERIALIZE(stream, IsSiloDamage);
	SERIALIZE(stream, IsCanUnitRepair);
	SERIALIZE(stream, IsCanUnitReload);
	SERIALIZE(stream, IsFlat);
	SERIALIZE(stream, IsDockUnload);
	SERIALIZE(stream, IsRecoilless);
	SERIALIZE(stream, IsHasStupidGuardMode);
	SERIALIZE(stream, IsBridgeRepairHut);
	SERIALIZE(stream, IsGate);
	SERIALIZE(stream, IsSAM);
	SERIALIZE(stream, IsConstructionYard);
	SERIALIZE(stream, IsNukeSilo);
	SERIALIZE(stream, IsRefinery);
	SERIALIZE(stream, IsWeeder);
	SERIALIZE(stream, IsWeaponsFactory);
	SERIALIZE(stream, IsLaserFencePost);
	SERIALIZE(stream, IsLaserFence);
	SERIALIZE(stream, IsFirestormWall);
	SERIALIZE(stream, IsHospital);
	SERIALIZE(stream, IsArmory);
	SERIALIZE(stream, IsEMPulseCannon);
	SERIALIZE(stream, IsTickTank);
	SERIALIZE(stream, IsTurretAnimAVoxel);
	SERIALIZE(stream, IsCloakGenerator);
	SERIALIZE(stream, IsSensorArray);
	SERIALIZE(stream, IsICBMLauncher);
	SERIALIZE(stream, IsArtillary);
	SERIALIZE(stream, IsHelipad);
	SERIALIZE(stream, IsGDIBarracks);
	SERIALIZE(stream, IsNODBarracks);
	SERIALIZE(stream, SuperWeapon);
	SERIALIZE(stream, SuperWeapon2);
	SERIALIZE(stream, GateStages);
	SERIALIZE(stream, PowersUpToLevel);
	SERIALIZE(stream, VoxelBarrelFile);
	SERIALIZE(stream, VoxelBarrelScale);
	SERIALIZE(stream, VoxelBarrelOffsetToPitchPivotPoint);
	SERIALIZE(stream, VoxelBarrelOffsetToRotatePivotPoint);
	SERIALIZE(stream, VoxelBarrelOffsetToBuildingPivotPoint);
	SERIALIZE(stream, VoxelBarrelOffsetToBarrelEnd);
	SERIALIZE(stream, TurretChargeAnimRate);
	SERIALIZE(stream, StartPitch);
	SERIALIZE(stream, IsLimpetMine);
	SERIALIZE(stream, IsMobileWar);
	SERIALIZE(stream, IsMobileStealth);
	SERIALIZE(stream, IsJuggernaut);
	SERIALIZE(stream, IsCoreDefender);
	SERIALIZE(stream, IsBarrelAnimAVoxel);
	SERIALIZE(stream, IsTurretAnimExclusive);
	SERIALIZE(stream, UnusedBTypeBool1);
	SERIALIZE(stream, IsDamagedDoor);
	SERIALIZE(stream, IsInvisibleInGame);
	SERIALIZE(stream, IsTerrainPalette);
	SERIALIZE(stream, IsCanPlaceAnywhere);
	SERIALIZE(stream, IsExtraDamageStage);
	SERIALIZE(stream, CanAIBuildThis);
	SERIALIZE(stream, IsBaseDefense);
	SERIALIZE(stream, IsSortCameoAsBaseDefense);
	SERIALIZE(stream, CloakRadiusInCells);
	SERIALIZE(stream, IsDemandLoad);
	SERIALIZE(stream, IsDemandLoadBuildup);
	SERIALIZE(stream, IsFreeBuildup);
	SERIALIZE(stream, IsThreatRatingNode);
	SERIALIZE(stream, ProduceCashStartup);
	SERIALIZE(stream, IsProduceCashStartupOneTime);
	SERIALIZE(stream, ProduceCashAmount);
	SERIALIZE(stream, ProduceCashDelay);
	SERIALIZE(stream, ProduceCashBudget);
	SERIALIZE(stream, IsProduceCashResetOnCapture);
	SERIALIZE(stream, TheaterImageFile);
}
