...
/// </summary>
static void Submit_Rect(bgfx::ViewId view, bgfx::TextureHandle texture, float x, float y, float width, float height,
...
{
	Submit_Rect(view, texture, x, y, width, height, 1.0f, 1.0f, 0xFFFFFFFF,
...
static void Submit_CRT_Picture(bgfx::TextureHandle source, int destx, int desty, int destwidth, int destheight,
	unsigned int samplerflags, bool flipv)
{
	CRTWarp const glass = { CRT_BARREL };

	Submit_Rect(VIEW_PRESENT, source, (float)destx, (float)desty, (float)destwidth, (float)destheight,
...
void Backend_Present(void const * pixels, int pitch, int destx, int desty, int destwidth, int destheight, BackendScaleMode mode, bool upload)
{
	...
	// The film is part of the picture, so the mask goes over it too.
	if (_CRTEnabled) {
...
/// <summary>
/// Turns the CRT filter on or off. While it is off the present path submits nothing extra.
/// </summary>
void Backend_Set_CRT_Filter(bool enabled)
{
	...
	// The filter's passes read what the passes before them left in the target, so the
	// present view has to draw them in the order they were submitted in.
	if (_Initialized) {
		bgfx::setViewMode(VIEW_PRESENT, bgfx::ViewMode::Sequential);
	}
}
