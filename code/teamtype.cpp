...
void TeamTypeClass::Serialize(SaveStreamClass & stream)
{
	...
	SERIALIZE(stream, VeteranLevel);
	SERIALIZE(stream, IsLoadable);
	SERIALIZE(stream, IsFull);
	SERIALIZE(stream, IsAnnoyance);
	SERIALIZE(stream, IsGuardSlower);
	SERIALIZE(stream, IsRecruiter);
	SERIALIZE(stream, IsAutocreate);
	SERIALIZE(stream, IsPrebuilt);
	SERIALIZE(stream, IsReinforcable);
	SERIALIZE(stream, IsWhiner);
	SERIALIZE(stream, IsAggressive);
	SERIALIZE(stream, IsLooseRecruit);
	SERIALIZE(stream, IsSuicide);
	SERIALIZE(stream, IsDroppod);
	SERIALIZE(stream, IsDropship);
	SERIALIZE(stream, OnTransOnly);
	SERIALIZE(stream, RecruitPriority);
	SERIALIZE(stream, MaxAllowed);
	SERIALIZE(stream, Fear);
	SERIALIZE(stream, House);
	SERIALIZE(stream, TechLevel);
	SERIALIZE(stream, Tag);
	SERIALIZE(stream, Origin);
	SERIALIZE(stream, Number);
	SERIALIZE(stream, Script);
	SERIALIZE(stream, TaskForce);
	SERIALIZE(stream, Scope);
	SERIALIZE(stream, AvoidThreats);
	SERIALIZE(stream, IsIonImmune);
	SERIALIZE(stream, TransportsReturnOnUnload);
	SERIALIZE(stream, AreTeamMembersRecruitable);
	SERIALIZE(stream, IsBaseDefense);
	SERIALIZE(stream, OnlyTargetHouseEnemy);
}
