...
void BuildingClass::Serialize(SaveStreamClass & stream)
{
	...
	SERIALIZE(stream, BState);
	SERIALIZE(stream, QueueBState);
	SERIALIZE(stream, WhoLastHurtMe);
	SERIALIZE(stream, WhomToRepay);
	SERIALIZE(stream, LastStrength);
	SERIALIZE(stream, AnimToTrack);
	SERIALIZE(stream, PlacementDelay);
	SERIALIZE(stream, Anims);
	SERIALIZE(stream, Upgrades);
	SERIALIZE(stream, LastSuperWeaponIndex);
	SERIALIZE(stream, TurretIndex);
	SERIALIZE(stream, BuildingLight);
	SERIALIZE(stream, GateTimer);
	// LightSource -- the lighting is rebuilt from scratch, so Post_Load drops it.
	SERIALIZE(stream, LaserFenceFrame);
	SERIALIZE(stream, FirestormWallFrame);
	SERIALIZE(stream, BuildingStage);
	SERIALIZE(stream, LastRenderRect);
	SERIALIZE(stream, LastRenderCoord);
	SERIALIZE(stream, LastRenderOffset);
	SERIALIZE(stream, IsOn);
	SERIALIZE(stream, IsNominal);
	SERIALIZE(stream, IsToRebuild);
	SERIALIZE(stream, IsToRepair);
	SERIALIZE(stream, IsAllowedToSell);
	SERIALIZE(stream, IsReadyToCommence);
	SERIALIZE(stream, IsWrenchVisible);
	SERIALIZE(stream, IsGoingToBlow);
	SERIALIZE(stream, IsSurvivorless);
	SERIALIZE(stream, IsCharging);
	SERIALIZE(stream, IsCharged);
	SERIALIZE(stream, IsCaptured);
	SERIALIZE(stream, HasOpened);
	SERIALIZE(stream, UnusedBuildingBool1);
	SERIALIZE(stream, IsDamagedAnims);
	SERIALIZE(stream, IsFogged);
	SERIALIZE(stream, IsRepairing);
	SERIALIZE(stream, HasBuildupData);
	SERIALIZE(stream, IsPoweredOn);
	SERIALIZE(stream, CloakGeneratorState);
	SERIALIZE(stream, CurrentCloakRadius);
	SERIALIZE(stream, CloakFieldCells);
	SERIALIZE(stream, IsSensing);
	SERIALIZE(stream, TranslucencyLevel);
	SERIALIZE(stream, Brightness);
	SERIALIZE(stream, UpgradeLevel);
	SERIALIZE(stream, GateFrame);
	SERIALIZE(stream, ProduceCashTimer);
	SERIALIZE(stream, ProduceCashRemaining);
	SERIALIZE(stream, IsProduceCashStartupPaid);
}
