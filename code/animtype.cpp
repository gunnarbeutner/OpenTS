...
void AnimTypeClass::Serialize(SaveStreamClass & stream)
{
	...
	SERIALIZE(stream, LoopStart);
	SERIALIZE(stream, LoopEnd);
	SERIALIZE(stream, Stages);
	SERIALIZE(stream, Loops);
	SERIALIZE(stream, Sound);
	SERIALIZE(stream, ChainTo);
	SERIALIZE(stream, DetailLevel);
	SERIALIZE(stream, TranslucencyDetailLevel);
	SERIALIZE(stream, RandomLoopDelayMin);
	SERIALIZE(stream, RandomLoopDelayMax);
	SERIALIZE(stream, RandomRateMin);
	SERIALIZE(stream, RandomRateMax);
	...
	SERIALIZE(stream, TrailerSeperation);
	...
	SERIALIZE(stream, TiberiumSpawnType);
	SERIALIZE(stream, TiberiumSpreadRadius);
	SERIALIZE(stream, YSortAdjust);
	SERIALIZE(stream, YDrawOffset);
	SERIALIZE(stream, RunningFrames);
	SERIALIZE(stream, IsFlamingGuy);
	SERIALIZE(stream, IsVeins);
	SERIALIZE(stream, IsMeteor);
	SERIALIZE(stream, IsTiberiumChainReaction);
	SERIALIZE(stream, IsTiberium);
	SERIALIZE(stream, IsBouncer);
	SERIALIZE(stream, IsTiled);
	SERIALIZE(stream, IsShouldUseCellDrawer);
	SERIALIZE(stream, IsUseNormalLight);
	SERIALIZE(stream, IsDemandLoad);
	SERIALIZE(stream, IsFreeAfterPlaying);
	SERIALIZE(stream, IsAnimatedTiberium);
	SERIALIZE(stream, IsAltPalette);
	SERIALIZE(stream, IsNormalized);
	SERIALIZE(stream, IsGroundLayer);
	SERIALIZE(stream, IsFlat);
	SERIALIZE(stream, IsTranslucent);
	SERIALIZE(stream, IsScorcher);
	SERIALIZE(stream, IsFlameThrower);
	SERIALIZE(stream, IsCraterForming);
	SERIALIZE(stream, IsSticky);
	SERIALIZE(stream, IsPingPong);
	SERIALIZE(stream, IsReverse);
	SERIALIZE(stream, IsShouldFogRemove);
}
