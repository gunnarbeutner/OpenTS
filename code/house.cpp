...
void HouseClass::Serialize(SaveStreamClass & stream)
{
	...
	SERIALIZE(stream, HouseTags);
	SERIALIZE(stream, ConYards);
	...
	SERIALIZE(stream, ProductionMode);
	SERIALIZE(stream, ActLike);
	SERIALIZE(stream, IsHuman);
	SERIALIZE(stream, IsPlayerControl);
	SERIALIZE(stream, IsStarted);
	SERIALIZE(stream, IsAlerted);
	SERIALIZE(stream, IsAITriggersOn);
	SERIALIZE(stream, IsBaseBuilding);
	SERIALIZE(stream, IsDiscovered);
	SERIALIZE(stream, IsDefeated);
	SERIALIZE(stream, IsObserver);
	SERIALIZE(stream, IsToDie);
	SERIALIZE(stream, IsToWin);
	SERIALIZE(stream, IsToLose);
	SERIALIZE(stream, IsCivEvacuated);
	SERIALIZE(stream, FirestormDefenseActivated);
	SERIALIZE(stream, IsThreatRatingNodeActive);
	SERIALIZE(stream, IsRecalcNeeded);
	SERIALIZE(stream, IPAddress);
	SERIALIZE(stream, SquadID);
	SERIALIZE(stream, LostConnection);
	SERIALIZE(stream, SelectedPath);
	SERIALIZE(stream, Paths);
	SERIALIZE(stream, IsVisionary);
	SERIALIZE(stream, IsTiberiumShort);
	SERIALIZE(stream, IsSpied);
	SERIALIZE(stream, IsThieved);
	SERIALIZE(stream, DidRepair);
	SERIALIZE(stream, IsBuiltSomething);
	SERIALIZE(stream, IsResigner);
	SERIALIZE(stream, IsGiverUpper);
	SERIALIZE(stream, IsAllToHunt);
	SERIALIZE(stream, IsParanoid);
	SERIALIZE(stream, IsToLook);
	SERIALIZE(stream, IQ);
	SERIALIZE(stream, State);
	SERIALIZE(stream, SuperWeapon);
	SERIALIZE(stream, JustBuiltStructure);
	SERIALIZE(stream, JustBuiltInfantry);
	SERIALIZE(stream, JustBuiltUnit);
	SERIALIZE(stream, JustBuiltAircraft);
	SERIALIZE(stream, Blockage);
	SERIALIZE(stream, RepairTimer);
	SERIALIZE(stream, AlertTime);
	SERIALIZE(stream, BorrowedTime);
	SERIALIZE(stream, CreditsSpent);
	SERIALIZE(stream, HarvestedCredits);
	SERIALIZE(stream, StolenBuildingsCredits);
	SERIALIZE(stream, CurUnits);
	SERIALIZE(stream, CurBuildings);
	SERIALIZE(stream, CurInfantry);
	SERIALIZE(stream, CurAircraft);
	SERIALIZE(stream, Tiberium);
	SERIALIZE(stream, Credits);
	SERIALIZE(stream, Capacity);
	SERIALIZE(stream, Weed);
	SERIALIZE(stream, field_1BC);
	...
	// CapturedBuildings
	// TotalCrates
	SERIALIZE(stream, AircraftFactories);
	SERIALIZE(stream, InfantryFactories);
	SERIALIZE(stream, UnitFactories);
	SERIALIZE(stream, BuildingFactories);
	SERIALIZE(stream, Power);
	SERIALIZE(stream, Drain);
	SERIALIZE(stream, AircraftFactory);
	SERIALIZE(stream, InfantryFactory);
	SERIALIZE(stream, UnitFactory);
	SERIALIZE(stream, BuildingFactory);
	SERIALIZE(stream, FlagLocation);
	SERIALIZE(stream, FlagHome);
	SERIALIZE(stream, UnitsKilled);
	SERIALIZE(stream, UnitsLost);
	SERIALIZE(stream, BuildingsKilled);
	SERIALIZE(stream, BuildingsLost);
	SERIALIZE(stream, WhoLastHurtMe);
	SERIALIZE(stream, Center);
	SERIALIZE(stream, Radius);
	SERIALIZE(stream, ZoneInfo);
	SERIALIZE(stream, LATime);
	SERIALIZE(stream, LAEnemy);
	SERIALIZE(stream, ToCapture);
	SERIALIZE(stream, RadarSpied);
	SERIALIZE(stream, PointTotal);
	SERIALIZE(stream, PreferredTarget);
	SERIALIZE(stream, BQuantity);
	SERIALIZE(stream, UQuantity);
	SERIALIZE(stream, IQuantity);
	SERIALIZE(stream, AQuantity);
	SERIALIZE(stream, ABQuantity);
	SERIALIZE(stream, AUQuantity);
	SERIALIZE(stream, AIQuantity);
	SERIALIZE(stream, AAQuantity);
	SERIALIZE(stream, PBQuantity);
	SERIALIZE(stream, PUQuantity);
	SERIALIZE(stream, PIQuantity);
	SERIALIZE(stream, PAQuantity);
	SERIALIZE(stream, Attack);
	SERIALIZE(stream, Enemy);
	SERIALIZE(stream, AngerNodes);
	SERIALIZE(stream, ScoutNodes);
	SERIALIZE(stream, AITimer);
	SERIALIZE(stream, PickEnemyTimer);
	SERIALIZE(stream, BuildStructure);
	SERIALIZE(stream, BuildUnit);
	SERIALIZE(stream, BuildInfantry);
	SERIALIZE(stream, BuildAircraft);
	SERIALIZE(stream, RatioAITriggerTeam);
	SERIALIZE(stream, RatioTeamAircraft);
	SERIALIZE(stream, RatioTeamInfantry);
	SERIALIZE(stream, RatioTeamUnits);
	SERIALIZE(stream, BaseDefenseTeamCount);
	SERIALIZE(stream, DropshipLoadouts);
	SERIALIZE(stream, CurrentDropship);
	SERIALIZE(stream, HasCloakGenerator);
	SERIALIZE(stream, RemapColorRGB);
	SERIALIZE(stream, Base);
	SERIALIZE(stream, RecalcPower);
	SERIALIZE(stream, RecalcRadar);
	SERIALIZE(stream, EMPDest);
	SERIALIZE(stream, NukeDest);
	SERIALIZE(stream, Allies);
	SERIALIZE(stream, DamageTime);
	SERIALIZE(stream, TeamTime);
	SERIALIZE(stream, TriggerTime);
	SERIALIZE(stream, SpeakAttackDelay);
	SERIALIZE(stream, SpeakPowerDelay);
	SERIALIZE(stream, SpeakMoneyDelay);
	SERIALIZE(stream, SpeakMaxedDelay);

	// BuildChoice -- a scratch list shared by every house rather than owned by one.
	SERIALIZE(stream, Regions);
	...
	// placed.
	SERIALIZE(stream, field_10E20);
	SERIALIZE(stream, field_10E28);
	SERIALIZE(stream, DefenseCostMultiplier);
	SERIALIZE(stream, field_10E38);
	SERIALIZE(stream, EnemyArmorForcePrediction);
	SERIALIZE(stream, EnemyAirForcePrediction);
	SERIALIZE(stream, EnemyInfantryForcePrediction);
	SERIALIZE(stream, PowerSurplus);
	SERIALIZE(stream, SpawnWaypoint);
}
