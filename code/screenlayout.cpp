...
{
	// A design space the surface cannot hold is clipped rather than fitted, which leaves a
	// screen placing its artwork against one rectangle and drawing it at the scale of
	// another. Nothing downstream can tell that happened, so it is said here.
	Rect const surface = (HiddenSurface != NULL) ? HiddenSurface->Get_Rect() : VisibleRect;

	if (size.X > surface.Width || size.Y > surface.Height) {
		DebugString("Shell: design %dx%d does not fit the %dx%d surface and will be clipped\n",
			size.X, size.Y, surface.Width, surface.Height);
	}
