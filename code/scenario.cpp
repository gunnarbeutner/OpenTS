...
void ScenarioClass::Save(SaveStreamClass & stream) const
{
	 ...
	 */
	stream.Body([&]{ const_cast<ScenarioClass *>(this)->Serialize(stream); });
...
{
	SERIALIZE(stream, Special);
	SERIALIZE(stream, NextScenarioName);
	SERIALIZE(stream, AltNextScenarioName);
	SERIALIZE(stream, Home);
	SERIALIZE(stream, AltHome);
	SERIALIZE(stream, UniqueID);
	SERIALIZE(stream, RandomNumber);
	SERIALIZE(stream, Difficulty);
	SERIALIZE(stream, CDifficulty);
	SERIALIZE(stream, ElapsedTimer);
	SERIALIZE(stream, Waypoint);
	SERIALIZE(stream, MissionTimer);
	SERIALIZE(stream, ShroudTimer);
	SERIALIZE(stream, FogTimer);
	SERIALIZE(stream, IceGrowthTimer);
	SERIALIZE(stream, VeinGrowthTimer);
	SERIALIZE(stream, AmbientChangeTimer);
	SERIALIZE(stream, Scenario);
	SERIALIZE(stream, Theater);
	SERIALIZE(stream, ScenarioName);
	SERIALIZE(stream, Description);
	SERIALIZE(stream, IntroMovie);
	SERIALIZE(stream, BriefMovie);
	SERIALIZE(stream, WinMovie);
	SERIALIZE(stream, LoseMovie);
	SERIALIZE(stream, ActionMovie);
	SERIALIZE(stream, PostScoreMovie);
	SERIALIZE(stream, PreMapSelectMovie);
	SERIALIZE(stream, BriefingText);
	SERIALIZE(stream, TransitTheme);
	SERIALIZE(stream, PlayerHouse);
	SERIALIZE(stream, PlayerSide);
	SERIALIZE(stream, LoadScreen);
	SERIALIZE(stream, LoadScreenX);
	SERIALIZE(stream, LoadScreenY);
	SERIALIZE(stream, CarryOverPercent);
	SERIALIZE(stream, CarryOverCap);
	SERIALIZE(stream, Percent);
	SERIALIZE(stream, GlobalFlags);
	SERIALIZE(stream, LocalFlags);
	SERIALIZE(stream, Views);
	SERIALIZE(stream, BridgeCount);
	SERIALIZE(stream, IsFreeRadar);
	SERIALIZE(stream, IsTrainCargo);
	SERIALIZE(stream, IsTibGrowth);
	SERIALIZE(stream, IsVeinGrowth);
	SERIALIZE(stream, IsIceGrowth);
	SERIALIZE(stream, IsBridgeChanged);
	SERIALIZE(stream, IsGlobalChanged);
	SERIALIZE(stream, IsAmbientLightChanged);
	SERIALIZE(stream, IsEndOfGame);
	SERIALIZE(stream, IsInheritTimer);
	SERIALIZE(stream, IsSkipScore);
	SERIALIZE(stream, IsOneTimeOnly);
	SERIALIZE(stream, IsNoMapSel);
	SERIALIZE(stream, IsTruckCrate);
	SERIALIZE(stream, IsMoneyTiberium);
	SERIALIZE(stream, IsTiberiumDeathToVisceroid);
	SERIALIZE(stream, IsIgnoreGlobalAITriggers);
	SERIALIZE(stream, IsMultiplayerOnly);
	SERIALIZE(stream, IsRandom);
	SERIALIZE(stream, IsCrateBeenPickedUp);
	SERIALIZE(stream, FadeTimer);
	SERIALIZE(stream, Campaign);
	SERIALIZE(stream, StartingDropships);
	SERIALIZE(stream, AllowableUnits);
	SERIALIZE(stream, AllowableUnitMaximums);
	SERIALIZE(stream, AllowableUnitCounts);
	SERIALIZE(stream, AmbientLight);
	SERIALIZE(stream, CurrentAmbientLight);
	SERIALIZE(stream, DesiredAmbientLight);
	SERIALIZE(stream, RedTint);
	SERIALIZE(stream, GreenTint);
	SERIALIZE(stream, BlueTint);
	SERIALIZE(stream, GroundLight);
	SERIALIZE(stream, LevelLight);
	SERIALIZE(stream, IonAmbientLight);
	SERIALIZE(stream, IonRedTint);
	SERIALIZE(stream, IonGreenTint);
	SERIALIZE(stream, IonBlueTint);
	SERIALIZE(stream, IonGroundLight);
	SERIALIZE(stream, IonLevelLight);
	SERIALIZE(stream, IsReadingScenario);
	SERIALIZE(stream, InitTime);
	SERIALIZE(stream, RequiredAddOn);
	SERIALIZE(stream, SpeechSide);
	SERIALIZE(stream, Stage);
	SERIALIZE(stream, IsInputLocked);
	SERIALIZE(stream, IsMPAIBaseNodes);
