...
void TechnoTypeClass::Serialize(SaveStreamClass & stream)
{
	...
	SERIALIZE(stream, WalkRate);
	SERIALIZE(stream, VeteranAbilities);
	SERIALIZE(stream, EliteAbilities);
	SERIALIZE(stream, SpecialThreatValue);
	SERIALIZE(stream, MyEffectivenessCoefficient);
	SERIALIZE(stream, TargetEffectivenessCoefficient);
	SERIALIZE(stream, TargetSpecialThreatCoefficient);
	SERIALIZE(stream, TargetStrengthCoefficient);
	SERIALIZE(stream, TargetDistanceCoefficient);
	SERIALIZE(stream, ThreatAvoidanceCoefficient);
	SERIALIZE(stream, SlowdownDistance);
	SERIALIZE(stream, DeaccelerationFactor);
	SERIALIZE(stream, AccelerationFactor);
	SERIALIZE(stream, CloakingSpeed);
	SERIALIZE(stream, DebrisTypes);
	SERIALIZE(stream, DebrisMaximums);
	...
	SERIALIZE(stream, Weight);
	SERIALIZE(stream, PhysicalSize);
	SERIALIZE(stream, InitialMission);
	SERIALIZE(stream, RollAngle);
	SERIALIZE(stream, PitchSpeed);
	SERIALIZE(stream, PitchAngle);
	SERIALIZE(stream, BuildLimit);
	SERIALIZE(stream, Category);
	SERIALIZE(stream, Unused2);
	SERIALIZE(stream, DeployTime);
	SERIALIZE(stream, FireAngle);
	SERIALIZE(stream, PipScale);
	SERIALIZE(stream, MaxPips);
	SERIALIZE(stream, Dock);
	SERIALIZE(stream, DeploysInto);
	SERIALIZE(stream, UndeploysInto);
	SERIALIZE(stream, UnloadingClass);
	SERIALIZE(stream, VoiceSelect);
	SERIALIZE(stream, VoiceMove);
	SERIALIZE(stream, VoiceAttack);
	SERIALIZE(stream, VoiceDie);
	SERIALIZE(stream, VoiceFeedback);
	SERIALIZE(stream, AuxSound1);
	SERIALIZE(stream, AuxSound2);
	SERIALIZE(stream, MZone);
	SERIALIZE(stream, ThreatRange);
	SERIALIZE(stream, MaxDebris);
	SERIALIZE(stream, MaxPassengers);
	SERIALIZE(stream, Size);
	SERIALIZE(stream, SizeLimit);
	SERIALIZE(stream, IsVehicleTransport);
	SERIALIZE(stream, SightRange);
	SERIALIZE(stream, Cost);
	SERIALIZE(stream, FlightLevel);
	SERIALIZE(stream, Level);
	SERIALIZE(stream, Prerequisite);
	SERIALIZE(stream, Risk);
	SERIALIZE(stream, Reward);
	SERIALIZE(stream, MaxSpeed);
	SERIALIZE(stream, Speed);
	SERIALIZE(stream, MaxAmmo);
	SERIALIZE(stream, Ownable);
	SERIALIZE(stream, IsAllowedToStartInMultiplayer);
	SERIALIZE(stream, CameoFilename);
	...
	SERIALIZE(stream, TurretOffset);
	SERIALIZE(stream, Points);
	SERIALIZE(stream, Explosion);
	SERIALIZE(stream, ScrapExplosion);
	SERIALIZE(stream, NaturalParticleSystem);
	SERIALIZE(stream, NaturalParticleLocation);
	SERIALIZE(stream, DamageParticleSystems);
	SERIALIZE(stream, DamageSmokeOffset);
	SERIALIZE(stream, ShadowIndex);
	SERIALIZE(stream, Capacity);
	SERIALIZE(stream, TurretNotExportedOnGround);
	SERIALIZE(stream, Weapons);
	SERIALIZE(stream, IsTypeImmune);
	SERIALIZE(stream, IsDetectDisguise);
	SERIALIZE(stream, IsMoveToShroud);
	SERIALIZE(stream, IsTrainable);
	SERIALIZE(stream, IsDamageSparks);
	SERIALIZE(stream, IsTargetLaser);
	SERIALIZE(stream, IsImmuneToVeins);
	SERIALIZE(stream, IsImmuneToEMP);
	SERIALIZE(stream, IsTiberiumHeal);
	SERIALIZE(stream, IsCloakStop);
	SERIALIZE(stream, IsTrain);
	SERIALIZE(stream, IsDropship);
	SERIALIZE(stream, IsToProtect);
	SERIALIZE(stream, IsDisableable);
	SERIALIZE(stream, IsUnbuildable);
	SERIALIZE(stream, IsDoubleOwned);
	SERIALIZE(stream, IsInvisible);
	SERIALIZE(stream, IsRadarVisible);
	SERIALIZE(stream, IsLeader);
	SERIALIZE(stream, IsScanner);
	SERIALIZE(stream, IsNominal);
	SERIALIZE(stream, IsTurretEquipped);
	SERIALIZE(stream, IsRepairable);
	SERIALIZE(stream, IsCrew);
	SERIALIZE(stream, IsRemappable);
	SERIALIZE(stream, IsCloakable);
	SERIALIZE(stream, IsSelfHealing);
	SERIALIZE(stream, SelfHealingStep);
	SERIALIZE(stream, SelfHealingRate);
	SERIALIZE(stream, SelfHealingCap);
	SERIALIZE(stream, IsMechanic);
	SERIALIZE(stream, IsOmniHealer);
	SERIALIZE(stream, IsExploding);
	SERIALIZE(stream, IsNoAutoFire);
	SERIALIZE(stream, IsRadarEquipped);
	SERIALIZE(stream, IsRegulated);
	SERIALIZE(stream, IsManualReload);
	SERIALIZE(stream, IsVisibleLoad);
	SERIALIZE(stream, IsLightningRod);
	SERIALIZE(stream, IsHunterSeeker);
	SERIALIZE(stream, IsCrusher);
	SERIALIZE(stream, IsTiltsWhenCrushes);
	SERIALIZE(stream, IsSubterranean);
	SERIALIZE(stream, IsAutoCrush);
	SERIALIZE(stream, IsAccelerates);
	SERIALIZE(stream, ZFudgeCliff);
	SERIALIZE(stream, ZFudgeColumn);
	SERIALIZE(stream, ZFudgeTunnel);
	SERIALIZE(stream, ZFudgeBridge);
}
