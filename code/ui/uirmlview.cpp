...
void UIRmlViewClass::Bind_Actions(Rml::DataModelConstructor & constructor)
{
	constructor.BindEventCallback("act", &UIRmlViewClass::Dispatch_Action, this);
}


void UIRmlViewClass::On_Action(Rml::Event &, Rml::VariantList const &)
{
}


void UIRmlViewClass::Dispatch_Action(Rml::DataModelHandle, Rml::Event & event, Rml::VariantList const & arguments)
{
	if (arguments.empty()) {
		return;
	}

	// A control reporting the value the model pushed into it is not an act of the player's.
	// RmlUi declares its change event uninterruptible, so the push cannot be stopped where it
	// is raised and is told from a player's own move here, once, for every screen.
	if (event.GetId() == Rml::EventId::Change && UI_Views_Are_Updating()) {
		return;
	}
...
void UIRmlViewClass::Process_Capture(Rml::Event & event)
{
	...
	// to the value it holds is not a move and is not clicked for. The event itself cannot be
	// stopped here, since RmlUi declares change uninterruptible; Dispatch_Action is where a
	// push is kept from reaching a screen.
	if (event.GetId() == Rml::EventId::Change) {
		if (target != nullptr && target->GetTagName() == "input"
			...
			if (event.GetParameter<float>("value", 0.0f) != target->GetAttribute<float>("value", 0.0f)
					&& !target->HasAttribute("data-silent")) {
