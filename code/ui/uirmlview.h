...
namespace Rml { class DataModelConstructor; class DataModelHandle; class ElementDocument; }
...
class UIRmlViewClass : public Rml::EventListener
{
	...
	protected:
		...
		// Binds `act(...)`, which is how a document raises an action, to On_Action. A screen
		// calls this from Bind_Model rather than binding a callback of its own, so that every
		// action arrives through the one place that decides what a screen may act on.
		void Bind_Actions(Rml::DataModelConstructor & constructor);

		// What an `act(...)` call in the document means to this screen. The first argument
		// names the action; a screen that needs more of them checks for itself. Only calls a
		// player made reach here.
		virtual void On_Action(Rml::Event & event, Rml::VariantList const & arguments);
	...
	private:
		...
		// What RmlUi calls for `act(...)`. It drops what no screen may act on and hands the
		// rest to On_Action.
		void Dispatch_Action(Rml::DataModelHandle model, Rml::Event & event, Rml::VariantList const & arguments);
