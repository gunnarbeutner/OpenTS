...
/// <summary>Takes on the end of the file the reading has been found in.</summary>
void BlockReadAheadClass::Bound(std::uint64_t stop)
{
	Stop = stop;
}
	...
	std::uint64_t & start, std::uint64_t & count) const
{
	...
	// Neither run reaches further in front of the reading than the reading has
	// covered: an undeclared one as far, a declared one BOUND_READS times as far
	// and from its first read rather than its second.
	...
	if (!Bounded()) {
		...
		// A declaration says where a file ends, not that the file is going to be
		// read through. The file layer declares the whole extent of an embedded
		// file whenever it is opened, and registering an archive nested inside
		// another opens one to read a few hundred bytes of directory and stops,
		// so a window sized by the link rather than by the reading sweeps a
		// megabyte or more of an archive nobody is reading. The reader's own
		// last read is still the floor, since a window narrower than that never
		// gets in front of it whatever the link was measured to be worth.
		if (reach > covered * (std::uint64_t)BOUND_READS) reach = covered * (std::uint64_t)BOUND_READS;
		if (reach < Wide * (std::uint64_t)BOUND_READS) reach = Wide * (std::uint64_t)BOUND_READS;
...
bool BlockReadRunsClass::Note(std::uint64_t first, std::uint64_t last, std::uint64_t & lost, std::uint64_t & stop)
{
	...
	// A run's end is the narrowest declaration covering the read it is at
	// rather than the one it was begun with: the file layer settles a bias in
	// several steps, and a run carries on out of one embedded file into the
	// next, so an end kept from the start would be read past. A read no
	// declaration covers leaves the end alone, since a declaration is only
	// remembered for a while.
	std::uint64_t const within = Bound(first);

	if (within != 0) Runs[Order[0]].Bound(within);
...
void HttpBlockSourceClass::Hint(BlockHintType kind, std::uint64_t offset, std::uint64_t length)
{
	...
	// An extent covering the whole image is hinted every time the image is
	// opened, and the engine opens an archive far more often than it reads one
	// through, so it says nothing about where the reading is going. Taken as a
	// run it would put every first read of an archive, however few bytes it
	// wants, at the front of a whole window.
	if (offset == 0 && length >= Length) return;
...
/// <summary>How far from one block the store answers a read the same way, held
/// or missing, so a read can be cut where what is held ends.</summary>
/// <returns>The blocks, never more than count and never zero unless count
/// is.</returns>
std::uint64_t HttpBlockSourceClass::Held_Run(std::uint64_t index, std::uint64_t count) const
{
	if (count == 0) return(0);

	bool const held = Index.Holds(index);
	std::uint64_t length = 1;

	while (length < count && Index.Holds(index + length) == held) length++;
...
bool HttpBlockSourceClass::Read_At(std::uint64_t offset, void * buffer, unsigned int length)
{
	...
	while (remaining > 0) {
		...
		if (within == 0 && remaining >= (unsigned int)BLOCK_SIZE && index < whole) {
			...
			// A span the store holds only the front of is cut where that ends,
			// so the held part is served locally and the request pays for the
			// rest. Asking for the whole span would fetch again what an earlier
			// window already brought down.
			if (Store_Ready()) count = Held_Run(index, count);
