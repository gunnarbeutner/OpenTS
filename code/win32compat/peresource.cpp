...
// Walks the resource directory of a Portable Executable image; a directory not
// yet placed in an image is walked with its addresses counted from its own
// start.
...
// Header fields are read one at a time so the reader does not depend on a
// compiler's packing of the Windows declarations.
...
// The language libraries are Windows-1252; a character it has no byte for
...
// Truncates rather than overruns; the result is terminated whenever there is
// room.
...
// Windows matches resource names without regard to case.
...
// A version resource is a tree of length-prefixed nodes, each a UTF-16 key, an
// optional value, and its children; every node and value starts on a four-byte
// boundary.
...
// A null key selects the first child of any name.
...
// The copy starts past a lead of zeroes because the walk uses offset zero to
...
// Zero means the address falls outside every section.
std::size_t PEResourceClass::Offset_For_RVA(std::uint32_t rva) const
{
	...
	for (unsigned int index = 0; index < SectionCount; index++) {
		...
		if (rva >= address && rva - address < span) {
			...
			// A section's tail past its raw size reads as zeroes, not as the
...
// Named entries come first and numbered ones after them.
	...
	std::size_t * size) const
{
	...
	// Each shipped language library holds one translation, so the first
	// language entry is the wanted one.
...
// The string table is stored in bundles of sixteen; within a bundle each string
// is a length in characters followed by that many characters, present or not.
...
bool PEResourceClass::Fetch_Version_String(char const * key, char * buffer, int size) const
{
	...
	// The first translation is the one the Windows build asks for.
	...
	// A text value counts its own terminator, which stops the copy early.
