...
// The stock control classes, as models. ownrdraw.cpp displaces each class
// procedure, paints the control itself, and keeps calling the displaced
// procedure for the state to draw; a class here still reports its own state
// changes and answers WM_NCHITTEST as Windows does.
...
// The first thirty-two bytes of window extra are the dialog words on every
// window, so the state pointer sits above them.
static int const CONTROL_STATE_OFFSET = 32;

// win32compat.h declares none of these.
static UINT const BST_PUSHED_STATE = 0x0004;
static UINT const STN_CLICKED_CODE = 0;
static UINT const DLGC_STATIC_CODE = 0x0100;

// Windows measures the font; ownrdraw.cpp overwrites this when it subclasses.
static int const DEFAULT_ITEM_HEIGHT = 13;

// The front end paints its own frame, and this is what it paints.
static int const COMBO_BOX_FRAME = 4;


// One shape for every class; a dialog holds too few controls for a per-class
// allocation to buy anything.
struct ControlState
{
	int Check;
	bool Pressed;
...
// Handles WM_CREATE and WM_NCDESTROY; true when the message was one of them.
...
// This is the whole of the button's drawing: ownrdraw.cpp paints from its own
// artwork when the parent hands the item back down, and the state carried here
// picks the artwork.
...
// The automatic styles carry their own state, so a check box toggles before the
...
static void Click_Button(HWND window, ControlState * state, LONG style)
{
	switch (style & 0x0F) {
		...
		case BS_AUTORADIOBUTTON: {
			// Group boundaries are not tracked: no shipped template holds a
			// radio button, so the siblings of one are the whole of its group.
			HWND parent = GetParent(window);
...
static LRESULT CALLBACK Button_Proc(HWND window, UINT message, WPARAM wparam, LPARAM lparam)
{
	...
	switch (message) {
		case WM_NCHITTEST:
			// A group box frames other controls, so a click on it belongs to
			// what it frames.
...
static LRESULT CALLBACK Edit_Proc(HWND window, UINT message, WPARAM wparam, LPARAM lparam)
{
	...
	switch (message) {
		...
		case WM_GETDLGCODE: {
			LRESULT code = DLGC_WANTCHARS | DLGC_WANTARROWS | DLGC_HASSETSEL;

			// A multiline edit takes the return key, or the dialog claims it
			// for the default button and a caller that acts on it, such as the
			// chat field, is never told.
			if ((GetWindowLongA(window, GWL_STYLE) & ES_MULTILINE) != 0) {
		...
		case WM_SETFOCUS: {
			...
			// A height of zero leaves the bar to whoever paints the field, the
			// only thing that knows what the text was drawn with.
			RECT client = {0, 0, 0, 0};
			GetClientRect(window, &client);
			CreateCaret(window, nullptr, 0, client.bottom - client.top);
			ShowCaret(window);
...
// exact asks for the whole text to match; otherwise only its beginning must.
...
// One line of the dialog font, which the dialog base units measure, plus the
// frame around it.
...
// Windows keeps only the closed box as the window and reports the template's
// height through CB_GETDROPPEDCONTROLRECT; the front end and windlg.cpp's
// layout pass read that rectangle.
...
static LRESULT CALLBACK Combo_Box_Proc(HWND window, UINT message, WPARAM wparam, LPARAM lparam)
{
	...
	// A list style combo box's window text is its selection; ownrdraw.cpp
	// paints the closed box out of GetWindowText.
	...
	switch (message) {
		...
		case WM_SETTEXT:
			// Windows selects the matching item; a list cannot show free text.
			if (listonly) {
		...
		case CB_SETITEMHEIGHT:
			// Windows measures the closed box and the rows apart; the front end
			// only asks the two to agree.
			state->ItemHeight = (int)lparam;
			return(0);
		...
		case CB_GETDROPPEDCONTROLRECT: {
			// The box plus its dropped list: the height the template asked for.
			RECT * rect = (RECT *)lparam;
			if (rect != nullptr) {
		...
		case WM_SIZE: {
			// A layout pass sizes the whole dropped rectangle, as on Windows;
			// the size this control gives itself is not one of those.
...
static LRESULT CALLBACK Track_Bar_Proc(HWND window, UINT message, WPARAM wparam, LPARAM lparam)
{
	...
	switch (message) {
		...
		case TBM_SETTICFREQ:
			// The engine draws its own slider, so ticks have nowhere to go.
			return(0);
...
// ownrdraw.cpp compares GetClassName's answer case sensitively against
// WC_BUTTON and its neighbors, so these are those spellings rather than the
// uppercase forms a template writes.
struct StockClass
{
	char const * Name;
	WNDPROC Procedure;
