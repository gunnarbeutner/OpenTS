...
// Exercises the read-only half of the WebAssembly target's file layer: the
// branch of PlatformFileClass and Platform_Find_Files that answers out of the
// archives an asset manifest names rather than out of the host directory. The
// release is served out of memory by a stand-in for XMLHttpRequest, so no
// server, document, game data or original executable is involved, and none
// may become one.
//
// What a request makes of an answer once it has one is covered the same way,
// against a stub standing in for the transport, since deciding which image
// was answered for is the harness's own arithmetic and not the network's.
...
/*
**	Reads a whole file through the file layer under test.
*/
bool Read_Through_Handle(char const * name, std::vector<unsigned char> & out)
{
	PlatformFileClass file;

	if (!file.Open(name, PlatformOpenType::READ)) return false;

	std::uint32_t const size = (std::uint32_t)file.Size();

	out.assign(size != 0 ? size : 1, 0);

	std::uint32_t read = 0;
	bool const ok = file.Read(out.data(), size, read) && read == size;
...
std::vector<std::string> Search(char const * pattern)
{
	...
	for (PlatformFileInfoType const & entry : Platform_Find_Files(pattern)) {
...
// Registers one archive of the release the stand-in below serves.
void Serve_File(char const * name, unsigned char const * data, unsigned int size)
{
	EM_ASM({
		globalThis.__opentsRelease = globalThis.__opentsRelease || ({});
		globalThis.__opentsRelease[UTF8ToString($0)] = HEAPU8.slice($1, $1 + $2);
	}, name, data, size);
}


// Serves assets.json, the manifest it points at, and ranges of every
// registered archive out of memory. Object literals are parenthesized
// because the block is a macro argument and a bare comma would end it.
void Serve_Release(void)
{
	EM_ASM({
		var base = "http://127.0.0.1:9/release/";
		var files = globalThis.__opentsRelease || ({});
		var records = [];

		for (var name in files) {
			records.push(({name: name, path: "files/" + name.toLowerCase(), sha256: "", size: files[name].length}));
		}

		var manifest = JSON.stringify(({format: "opents-web-assets-v2", files: records}));
		var pointer = JSON.stringify(({format: "opents-web-assets-pointer-v1", manifest: "manifest.json"}));

		globalThis.location = {href: base};
		Module["opentsManifestBase"] = base;
		...
		globalThis.XMLHttpRequest = function () {
			...
			answer.responseText = "";
			answer.responseURL = "";
			answer.open = function (method, where) { url = "" + where; };
			...
			answer.getResponseHeader = function (name) {
				...
				if (wanted === "content-range" && first >= 0) return "bytes " + first + "-" + last + "/" + total;
				return null;
			};
			answer.send = function () {
				answer.responseURL = url;

				if (url === base + "assets.json") {
					answer.status = 200;
					answer.responseText = pointer;
					return;
				}
				if (url === base + "manifest.json") {
					answer.status = 200;
					answer.responseText = manifest;
					return;
				}

				for (var name in files) {
					if (url !== base + "files/" + name.toLowerCase()) continue;
					...
					last = Math.min(last, bytes.length - 1);
					answer.response = bytes.slice(first, last + 1).buffer;
					answer.status = 206;
					return;
				}
...
{
	std::string const key = BlockIndexClass::Signature("http://host/opents-data.iso", 4096);

	Check(!key.empty(), "an image that can be identified has a key");
	Check(BlockIndexClass::Signature("http://host/opents-data.iso", 4096) == key,
		"the same image answers to the same key");
	Check(BlockIndexClass::Signature("http://host/other.iso", 4096) != key,
		"a different URL is a different image");
	Check(BlockIndexClass::Signature("http://host/opents-data.iso", 8192) != key,
		"a different length is a different image");

	Check(BlockIndexClass::Signature(nullptr, 4096).empty()
		&& BlockIndexClass::Signature("", 4096).empty()
		&& BlockIndexClass::Signature("http://host/opents-data.iso", 0).empty(),
		"an image that cannot be identified has no key, and so no store");

	std::string const flattened = BlockIndexClass::Signature("http://host/x\ny\tz\x80.iso", 1);
	...
	*/
	Check(BlockIndexClass::Store_Slot("http://host/ts1.iso")
		!= BlockIndexClass::Store_Slot("http://host/ts2.iso"),
		"two images are held in slots of their own");
	Check(BlockIndexClass::Store_Slot("http://host/ts1.iso")
		== BlockIndexClass::Store_Slot("http://host/ts1.iso"),
		"and one image answers to the same slot however it is identified");
	Check(BlockIndexClass::Store_Slot(nullptr).empty() && BlockIndexClass::Store_Slot("").empty(),
	...
	*/
	...
	std::string const other = BlockIndexClass::Signature("http://host/opents-data.iso", 8192);
	...
	*/
	BlockIndexClass bounded;

	bounded.Reset(key);

	std::uint64_t const blocks = BlockIndexClass::STORE_LIMIT / 65536;
	...
	Check(bounded.Count() == (std::size_t)blocks && bounded.Bytes() == BlockIndexClass::STORE_LIMIT,
	...
	Check(bounded.Bytes() == BlockIndexClass::STORE_LIMIT, "leaving the store no larger than the cap");
	...
	*/
	evicted.clear();
	bounded.Note(blocks + 1, 65536, evicted, BlockIndexClass::ADMIT_GUESS);
	...
	evicted.clear();
	bounded.Note(blocks + 2, 65536, evicted, BlockIndexClass::ADMIT_READ);
	...
	*/
	BlockIndexClass full;
	std::uint64_t const most = BlockIndexClass::STORE_MAX / 65536;

	full.Reset(key);
	full.Cap(BlockIndexClass::STORE_MAX, evicted);
	...
	Check(large.size() < BlockIndexClass::RECORD_MAX,
		"and the record describing it fits what reads it back");

	BlockIndexClass reread;
...
// How often the page has been asked to reload since the server last changed.
int Reloaded(void)
{
	return(EM_ASM_INT({ return globalThis.__opentsReloaded || 0; }));
}
...
void Serve_Changed(char const * page, double total, char const * tag)
{
	EM_ASM({
		...
		globalThis.location.reload = function () {
			globalThis.__opentsReloaded = (globalThis.__opentsReloaded || 0) + 1;
		};
		globalThis.__opentsReloaded = 0;
...
void Check_Probe_Recall(void)
{
	...
	*/
	...
	Check(Reloaded() == 1,
		"and the page is asked to reload, so nothing read from the old image survives");
...
void Check_Stall_Record(void)
{
	...
	*/
	std::vector<unsigned char> whole(BLOCK_UNIT_SIZE);
...
void Check_Link(void)
{
	...
	Check(!link.Measured() && link.Window() == (unsigned int)BlockLinkClass::WINDOW_MIN,
		"a link nothing has been read over reaches no further than the floor");
	Check(link.Span() == (unsigned int)BlockLinkClass::SPAN_MIN,
		"and asks for no more than one span at a time");
	Check(link.Flights() == (unsigned int)BlockLinkClass::FLIGHTS_MIN,
	...
	**	almost nothing in front of the reading for a window to hide. A rate is read only out
	**	of a request that spent RATE_FLOOR moving, so the requests here carry a megabyte.
	*/
	BlockLinkClass local;

	for (unsigned int step = 0; step < 12; step++) {
		local.Note(2048, 0.5);
		local.Note(1048576, 11.0);
	}
	...
	Check(local.Span() == (unsigned int)BlockLinkClass::SPAN_MIN,
		"and a request asks for the same span it always did");
	Check(local.Flights() == (unsigned int)BlockLinkClass::FLIGHTS_MIN,
	...
	*/
	...
	Check(forty.Flights() == (unsigned int)BlockLinkClass::FLIGHTS_MIN,
	...
	*/
	BlockLinkClass distant;

	for (unsigned int step = 0; step < 20; step++) {
		distant.Note(2048, 150.0);
		distant.Note(1048576, 201.2);
	}
	...
	Check(distant.Window() <= (unsigned int)BlockLinkClass::WINDOW_MAX
		&& distant.Span() <= (unsigned int)BlockLinkClass::SPAN_MAX
		&& distant.Flights() <= (unsigned int)BlockLinkClass::FLIGHTS_MAX,
	...
	*/
	BlockLinkClass drift;

	for (unsigned int step = 0; step < 20; step++) {
		drift.Note(2048, 40.0);
		drift.Note(1048576, 91.2);
	}
...
void Check_Declared_Runs(void)
{
	...
	Check(!file.Span(blocks, window, span, start, count),
		"but nothing is asked for until the file has been read from at all");
	...
	*/
	...
	/*
	**	Neither run is reached in front of further than the reading has covered, because a
	**	declaration says where a file ends and not that the file is going to be read through:
	**	the engine declares an embedded file's whole extent whenever it opens one, and
	**	registering an archive nested inside another opens one to read a few hundred bytes of
	**	directory and stops. A declaration is still better evidence than a second read, so it
	**	buys BOUND_READS times the covering and it buys it from the first read.
	*/
	BlockReadAheadClass opened;
	...
	Check(opened.Edge() == 1 + (std::uint64_t)BlockReadAheadClass::BOUND_READS,
		"a declared file read once is reached in front of by that read several times over");
	Check(opened.Edge() < 1 + (std::uint64_t)window / 2,
		"rather than by a window the link was measured to be worth");

	BlockReadAheadClass noticed;
	...
	*/
	unsigned int const smallest = (unsigned int)BlockLinkClass::WINDOW_MIN;
	unsigned int const shortest = (unsigned int)BlockLinkClass::SPAN_MIN;

	BlockReadAheadClass movie;
	...
	Check(reached >= 5 + 5 * (std::uint64_t)BlockReadAheadClass::BOUND_READS / 2,
	...
	*/
	...
	/*
	**	An archive nested inside another is registered by reading its directory and then left
	**	alone, and the whole of it was declared by the open that preceded that read. What is
	**	fetched over it is worth the one read, where a window sized by the link swept the
	**	archive from its front for nothing.
	*/
	BlockReadRunsClass registered;

	registered.Declare(6000, 6033);
	registered.Note(6000, 6000, lost, stop);

	std::uint64_t swept = 0;

	while (registered.Current().Span(blocks, window, span, start, count)) {
		registered.Current().Issued(start + count);
		swept = start + count;
	}

	Check(swept == 6001 + (std::uint64_t)BlockReadAheadClass::BOUND_READS,
		"registering an archive nested inside another does not sweep the front of it");

	/*
	**	The narrower declaration often arrives after the reading has already begun, because
	**	the file layer settles a bias in several steps and the archive is being read all the
	**	while. The run therefore takes the end of whichever declaration covers the read it is
	**	at, not the one it happened to be started with, or an embedded file's last read would
	**	be followed by a window running on into the rest of the archive.
	*/
	BlockReadRunsClass settling;

	settling.Declare(0, 100000);
	settling.Note(10, 10, lost, stop);
	Check(settling.Current().Limit() == 100000, "a run begun with only the archive declared");

	settling.Declare(10, 90);
	settling.Note(11, 89, lost, stop);
	Check(settling.Current().Limit() == 90,
		"takes the embedded file's end as soon as one is declared over it");
	Check(!settling.Current().Span(blocks, window, span, start, count),
		"so a read to the end of that file is not fetched past it");

	/*
	**	Carrying on out of one embedded file into the next takes the next one's end, since
	**	the run is where the reading is rather than where it started.
	*/
	settling.Declare(90, 140);
	settling.Note(90, 90, lost, stop);
	Check(settling.Current().Limit() == 140, "and the next file's end on moving into it");
...
void Check_Read_Ahead(void)
{
	...
	*/
	...
	unsigned int const span = (unsigned int)BlockLinkClass::SPAN_MIN;

	BlockReadAheadClass ahead;
	...
	*/
	...
	Check(count == (std::uint64_t)BlockLinkClass::WINDOW_MIN,
	...
	*/
	...
	Check(ahead.Edge() - ahead.Cursor() > (std::uint64_t)BlockLinkClass::WINDOW_MIN,
...
void Check_Read_Runs(void)
{
	...
	*/
	...
	Check(lost == 10459 && stop == 10459 + (std::uint64_t)BlockLinkClass::WINDOW_MIN,
...
int main(int argc, char ** argv)
{
	...
	*/
	...
	// The archives the manifest names stay open for the rest of the process,
	// so the file layer is exercised last, where nothing else serves them.
	// CATALOG has no extension, which is what tells `*.*` apart from `*`;
	// TIBSUN.MIX and SIDEBAR.MIX are registered out of alphabetical order so
	// the search has something to sort.
	...
	Serve_File("TIBSUN.MIX", tibsun.data(), (unsigned int)tibsun.size());
	Serve_File("SIDEBAR.MIX", sidebar.data(), (unsigned int)sidebar.size());
	Serve_File("CATALOG", catalog.data(), (unsigned int)catalog.size());
	Serve_Release();

	std::vector<unsigned char> read;

	Check(Read_Through_Handle("TIBSUN.MIX", read), "a file the manifest names opens and reads");
	Check(read == tibsun, "the bytes read are the bytes served");

	Check(Read_Through_Handle("tibsun.mix", read) && read == tibsun, "the name matches without regard to case");
	Check(Read_Through_Handle(".\\TIBSUN.MIX", read) && read == tibsun, "either separator and a leading dot resolve");
	Check(!Read_Through_Handle("../TIBSUN.MIX", read), "a path climbing out of the directory is refused");
	Check(!Read_Through_Handle("NOTHERE.MIX", read), "a name on neither side is not found");

	/*
	**	The open file answers the rest of the file layer.
	*/
	PlatformFileClass archive;

	Check(archive.Open("TIBSUN.MIX", PlatformOpenType::READ), "an archive opens");
	Check(archive.Size() == (std::int64_t)tibsun.size(), "the size is the size served");

	unsigned char window[16];
	std::uint32_t got = 0;

	Check(archive.Seek(4096, SEEK_SET) == 4096, "a seek from the start reports the position");
	Check(archive.Read(window, sizeof(window), got) && got == sizeof(window), "a read after a seek delivers");
	Check(std::memcmp(window, tibsun.data() + 4096, sizeof(window)) == 0, "it delivers from where the seek put it");

	Check(archive.Seek(-16, SEEK_CUR) == 4096, "a relative seek moves from the position");
	Check(archive.Seek(0, SEEK_END) == (std::int64_t)tibsun.size(), "a seek to the end reports the size");
	Check(archive.Read(window, sizeof(window), got) && got == 0, "a read at the end succeeds with nothing");
	Check(archive.Seek(-1, SEEK_SET) == -1, "a seek before the start is refused");

	Check(archive.Seek((std::int64_t)tibsun.size() - 4, SEEK_SET) == (std::int64_t)tibsun.size() - 4,
		"the position can be put just short of the end");
	Check(archive.Read(window, sizeof(window), got) && got == 4, "a read across the end is short rather than failed");

	Check(!archive.Write(window, 4, got) && got == 0, "a write to the archive is denied");
	Check(!archive.Flush(), "flushing the archive is denied");
	Check(!archive.Set_Modified_Time(FileTimeType{}), "stamping the archive is denied");

	FileTimeType written;

	Check(archive.Modified_Time(written), "a date is reported, though a manifest records none");

	Check(archive.Close(), "the archive closes");
	Check(!archive.Close(), "and does not close twice");

	PlatformFileInfoType info;

	Check(Platform_File_Info("TIBSUN.MIX", info) && info.Size == tibsun.size() && info.IsReadOnly
		&& !info.IsDirectory, "a name only the manifest has reports a read only file of the right size");
	Check(!Platform_File_Info("NOTHERE.MIX", info), "a name neither side has reports nothing");
	...
	*/
	...
	Check(found.size() == 2 && found[0] == "SIDEBAR.MIX" && found[1] == "TIBSUN.MIX",
		"a wildcard search reports the manifest in case insensitive order");

	// `*.*` is DOS for every file, so it has to reach a name with no extension.
	// The host half of the same search reports the directory's own entries, so
	// what is asserted is that the manifest's extensionless name is among them.
	found = Search("*.*");
	...
	found = Search("CACHE/*.MIX");
	Check(found.empty(), "a search inside a directory reports nothing from the manifest, which has none");

	found = Search("*.NONE");
	Check(found.empty(), "a wildcard matching nothing reports nothing");

	// The host sits over the manifest. A file written beside it shadows the
	// one the manifest names, and a search reports the host's copy once.
	std::vector<unsigned char> const shadow = Pattern(9, 64);

	Check(Write_File("SIDEBAR.MIX", shadow), "a file is written beside the release");
	...
	Platform_Remove_File("SIDEBAR.MIX");
