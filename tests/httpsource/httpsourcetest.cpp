...
// branch of CreateFileA and the FindFirstFileA family that answers out of the
...
int main(int argc, char ** argv)
{
	...
	*/
	...
	Check(handle != INVALID_HANDLE_VALUE, "an archive handle is handed out");

	DWORD high = 0;

	Check(GetFileSize(handle, &high) == (DWORD)tibsun.size() && high == 0, "the size is the size served");
	...
	Check(GetFileTime(handle, nullptr, nullptr, &written) != FALSE,
		"a date is reported, though a manifest records none");
	...
	Check(GetFileAttributesA("SIDEBAR.MIX") == FILE_ATTRIBUTE_READONLY,
		"a name only the manifest has reports read only attributes");
