...
// The style that makes a combo box a list with no edit field. the substitute's windows.h has no
// need for it, and the SDK's supplies it on the other target.
...
static void Check_Window_Words(HINSTANCE instance, HWND frame)
{
	...
	Check("GWLP_HWNDPARENT reports the parent",
		(HWND)GetWindowLongPtrA(control, GWLP_HWNDPARENT) == frame);
	...
	/*
	** The dialog slots. The engine stores the address of a dialog's result in DWLP_USER on
	** windows whose class declared no window extra of its own, so the slots exist whatever
	** the class asked for.
	*/
	Check_Equal("DWLP_USER starts at zero", (long)GetWindowLongPtrA(control, DWLP_USER), 0);
	SetWindowLongPtrA(control, DWLP_USER, 0x1234);
	Check_Equal("DWLP_USER reads the value back", (long)GetWindowLongPtrA(control, DWLP_USER), 0x1234);

	SetWindowLongPtrA(control, DWLP_MSGRESULT, 0x2345);
	SetWindowLongPtrA(control, DWLP_DLGPROC, 0x3456);
	Check_Equal("DWLP_MSGRESULT keeps its own value",
		(long)GetWindowLongPtrA(control, DWLP_MSGRESULT), 0x2345);
	Check_Equal("DWLP_DLGPROC keeps its own value",
		(long)GetWindowLongPtrA(control, DWLP_DLGPROC), 0x3456);
	Check_Equal("DWLP_USER is not disturbed by its neighbors",
		(long)GetWindowLongPtrA(control, DWLP_USER), 0x1234);
	...
	*/
	...
	WNDPROC previous = (WNDPROC)SetWindowLongPtrA(control, GWLP_WNDPROC,
		(LONG_PTR)Second_Procedure);
	...
	SetWindowLongPtrA(control, GWLP_WNDPROC, (LONG_PTR)previous);
...
/*
** A character a software keyboard produced while correcting or composing arrives with no key
** press behind it, so the window manager is asked to carry it to whatever holds the focus.
** What is checked is that it reaches the field that wants one, that it arrives once, and
** that a control wanting none is left alone.
**
** Arriving once is the part worth pinning down: a pump translates a key press it is about
** to dispatch, so a press posted alongside the character would produce a second one.
*/
static void Check_Posted_Characters(HWND edit, HWND button)
{
	char text[32];

	Pump_Messages();

	Win32_User_Post_Character('a');
	Win32_User_Post_Character('b');
	Pump_Messages();

	text[0] = '\0';
	GetWindowTextA(edit, text, sizeof(text));
	Check("A character with no key behind it reaches the field that wants one",
		strcmp(text, "ab") == 0);
	...
	text[0] = '\0';
	GetWindowTextA(edit, text, sizeof(text));
	Check("A backspace with no key behind it erases from the field", strcmp(text, "a") == 0);

	// A real key press, which the pump turns into a character of its own on the way through.
	// The two checks above mean nothing without it: it is what would have doubled them.
	PostMessageA(edit, WM_KEYDOWN, 'D', 1);
	Pump_Messages();

	text[0] = '\0';
	GetWindowTextA(edit, text, sizeof(text));
	Check("A key press is translated into the character it produces", strcmp(text, "ad") == 0);

	SetFocus(button);
	Pump_Messages();

	Win32_User_Post_Character('c');

	MSG message;
	Check("A control that wants no characters is posted none",
		PeekMessageA(&message, button, WM_CHAR, WM_CHAR, PM_NOREMOVE) == FALSE);

	Pump_Messages();

	text[0] = '\0';
	GetWindowTextA(edit, text, sizeof(text));
	Check("Nothing reached the field the focus left", strcmp(text, "ad") == 0);
...
static void Check_Text_Input(HINSTANCE instance, HWND frame)
{
	...
	SetFocus(edit);
	Check_Posted_Characters(edit, button);
...
#if defined(OPENTS_WIN32_SUBSTITUTE)
/*
** The description an automated check reads the screen from: every visible window with its
** identifier, class, text and place, and none of the hidden ones. The rectangle is checked
** against where the child was put, since a check that clicks by description lands there.
*/
static void Check_Description(HINSTANCE instance, HWND frame)
{
	HWND shown = CreateWindowExA(0, "Button", "Say \"yes\"", WS_CHILD | WS_VISIBLE,
		10, 20, 100, 30, frame, (HMENU)(ULONG_PTR)5101, instance, NULL);
	HWND hidden = CreateWindowExA(0, "Button", "Never", WS_CHILD,
		10, 60, 100, 30, frame, (HMENU)(ULONG_PTR)5102, instance, NULL);

	Check("CreateWindowEx builds the controls to describe", shown != NULL && hidden != NULL);
	if (shown == NULL || hidden == NULL) {
		return;
	}

	std::string text;
	Win32_User_Describe(text);

	Check("A description is a JSON array", !text.empty() && text[0] == '[' && text.back() == ']');
	Check("A visible control is described with its identifier", text.find("\"id\":5101") != std::string::npos);
	Check("Its class is named", text.find("\"class\":\"Button\"") != std::string::npos);
	Check("Its text is carried with the quotes escaped", text.find("\"text\":\"Say \\\"yes\\\"\"") != std::string::npos);
	Check("A hidden control is left out", text.find("\"id\":5102") == std::string::npos);

	RECT where;
	Win32_User_Window_Rect(shown, &where);
	char expected[64];
	snprintf(expected, sizeof(expected), "\"rect\":[%ld,%ld,100,30]", (long)where.left, (long)where.top);
	Check("The rectangle is where the control was put", text.find(expected) != std::string::npos);
...
int main(void)
{
	...
	Check_Description(instance, frame);
#endif
