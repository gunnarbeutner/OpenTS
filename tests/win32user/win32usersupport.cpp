...
int Browser_Message_Box(char const *, char const *, int) { return(0); }
void Browser_Show_Cursor(void const *, char const *, unsigned long const *, int, int, int, int) {}
void Browser_Set_Event_Hook(BrowserEventHook) {}
...
unsigned short Browser_Event_Modifiers(void) { return(0); }
void Browser_Apply_Modifiers(unsigned short) {}


/*
** What a key produces, which the page answers for out of the layout it saw an event under.
** A harness has no layout and needs none: a letter key produces its lowercase letter and
** nothing else produces anything, which is enough for TranslateMessage to have something to
** translate and so for a check to tell one character from two.
*/
int Browser_Key_To_ASCII(unsigned short key)
{
	unsigned short code = (unsigned short)(key & 0xFF);
...
void Window_Point_To_Game(POINT &) {}
