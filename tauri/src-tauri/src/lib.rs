...
// A phone or a tablet has no window to resize; the page asks all the same.
#[cfg(mobile)]
#[tauri::command]
fn toggle_fullscreen() -> Result<(), String> {
    Ok(())
}
...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            let builder = WebviewWindowBuilder::new(app.handle(), "main", WebviewUrl::CustomProtocol(url))
                .title("OpenTS")
                .initialization_script(BRIDGE);

            // A window to open at on a desktop. A phone or a tablet has only the screen,
            // and asking for a size there makes the webview larger than it.
            #[cfg(desktop)]
